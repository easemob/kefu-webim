...
this._throttledRequestHandler()}if(this._requests.length>0){var time_elapsed=this._requests[0].age();null!==this._requests[0].dead&&this._requests[0].timeDead()>Math.floor(Strophe.SECONDARY_TIMEOUT*this.wait)&&this._throttledRequestHandler(),time_elapsed>Math.floor(Strophe.TIMEOUT*this.wait)&&(Strophe.warn("Request "+this._requests[0].id+" timed out, over "+Math.floor(Strophe.TIMEOUT*this.wait)+" seconds since last activity"),this._throttledRequestHandler())}}},_onRequestStateChange:function(func,req){if(Strophe.debug("request id "+req.id+"."+req.sends+" state changed to "+req.xhr.readyState),req.abort)return void(req.abort=!1);var reqStatus;if(4==req.xhr.readyState){reqStatus=0;try{reqStatus=req.xhr.status}catch(e){}if("undefined"==typeof reqStatus&&(reqStatus=0),this.disconnecting&&reqStatus>=400)return void this._hitError(reqStatus);var reqIs0=this._requests[0]==req,reqIs1=this._requests[1]==req;(reqStatus>0&&500>reqStatus||req.sends>5)&&(this._removeRequest(req),Strophe.debug("request id "+req.id+" should now be removed")),200==reqStatus?((reqIs1||reqIs0&&this._requests.length>0&&this._requests[0].age()>Math.floor(Strophe.SECONDARY_TIMEOUT*this.wait))&&this._restartRequest(0),Strophe.debug("request id "+req.id+"."+req.sends+" got 200"),func(req),this.errors=0):(Strophe.error("request id "+req.id+"."+req.sends+" error "+reqStatus+" happened"),(0===reqStatus||reqStatus>=400&&600>reqStatus||reqStatus>=12e3)&&(this._hitError(reqStatus),reqStatus>=400&&500>reqStatus&&(this._conn._changeConnectStatus(Strophe.Status.DISCONNECTING,null),this._conn._doDisconnect()))),reqStatus>0&&500>reqStatus||req.sends>5||this._throttledRequestHandler()}},_processRequest:function(i){var self=this,req=this._requests[i],reqStatus=-1;try{4==req.xhr.readyState&&(reqStatus=req.xhr.status)}catch(e){Strophe.error("caught an error in _requests["+i+"], reqStatus: "+reqStatus)}if("undefined"==typeof reqStatus&&(reqStatus=-1),req.sends>this._conn.maxRetries)return void this._conn._onDisconnectTimeout();var time_elapsed=req.age(),primaryTimeout=!isNaN(time_elapsed)&&time_elapsed>Math.floor(Strophe.TIMEOUT*this.wait),secondaryTimeout=null!==req.dead&&req.timeDead()>Math.floor(Strophe.SECONDARY_TIMEOUT*this.wait),requestCompletedWithServerError=4==req.xhr.readyState&&(1>reqStatus||reqStatus>=500);if((primaryTimeout||secondaryTimeout||requestCompletedWithServerError)&&(secondaryTimeout&&Strophe.error("Request "+this._requests[i].id+" timed out (secondary), restarting"),req.abort=!0,req.xhr.abort(),req.xhr.onreadystatechange=function(){},this._requests[i]=new Strophe.Request(req.xmlData,req.origFunc,req.rid,req.sends),req=this._requests[i]),0===req.xhr.readyState){Strophe.debug("request id "+req.id+"."+req.sends+" posting");try{req.xhr.open("POST",this._conn.service,this._conn.options.sync?!1:!0),req.xhr.setRequestHeader&&req.xhr.setRequestHeader("Content-Type","text/xml; charset=utf-8")}catch(e2){return Strophe.error("XHR open failed."),this._conn.connected||this._conn._changeConnectStatus(Strophe.Status.CONNFAIL,"bad-service"),void this._conn.disconnect()}var sendFunc=function(){if(req.date=new Date,self._conn.options.customHeaders){var headers=self._conn.options.customHeaders;for(var header in headers)headers.hasOwnProperty(header)&&req.xhr.setRequestHeader(header,headers[header])}req.xhr.send(req.data)};if(req.sends>1){var backoff=1e3*Math.min(Math.floor(Strophe.TIMEOUT*this.wait),Math.pow(req.sends,3));setTimeout(sendFunc,backoff)}else sendFunc();req.sends++,this._conn.xmlOutput!==Strophe.Connection.prototype.xmlOutput&&(req.xmlData.nodeName===this.strip&&req.xmlData.childNodes.length?this._conn.xmlOutput(req.xmlData.childNodes[0]):this._conn.xmlOutput(req.xmlData)),this._conn.rawOutput!==Strophe.Connection.prototype.rawOutput&&this._conn.rawOutput(req.data)}else Strophe.debug("_processRequest: "+(0===i?"first":"second")+" request has readyState of "+req.xhr.readyState)},_removeRequest:function(req){Strophe.debug("removing request");var i;for(i=this._requests.length-1;i>=0;i--)req==this._requests[i]&&this._requests.splice(i,1);req.xhr.onreadystatechange=function(){},this._throttledRequestHandler()},_restartRequest:function(i){var req=this._requests[i];null===req.dead&&(req.dead=new Date),this._processRequest(i)},_reqToData:function(req){try{return req.getResponse()}catch(e){if("parsererror"!=e)throw e;this._conn.disconnect("strophe-parsererror")}},_sendTerminate:function(pres){Strophe.info("_sendTerminate was called");var body=this._buildBody().attrs({type:"terminate"});pres&&body.cnode(pres.tree());var req=new Strophe.Request(body.tree(),this._onRequestStateChange.bind(this,this._conn._dataRecv.bind(this._conn)),body.tree().getAttribute("rid"));this._requests.push(req),this._throttledRequestHandler()},_send:function(){clearTimeout(this._conn._idleTimeout),this._throttledRequestHandler(),this._conn._idleTimeout=setTimeout(this._conn._onIdle.bind(this._conn),100)},_sendRestart:function(){this._throttledRequestHandler(),clearTimeout(this._conn._idleTimeout)},_throttledRequestHandler:function(){this._requests?Strophe.debug("_throttledRequestHandler called with "+this._requests.length+" requests"):Strophe.debug("_throttledRequestHandler called with undefined requests"),this._requests&&0!==this._requests.length&&(this._requests.length>0&&this._processRequest(0),this._requests.length>1&&Math.abs(this._requests[0].rid-this._requests[1].rid)<this.window&&this._processRequest(1))}},Strophe}),function(root,factory){return"function"==typeof define&&define.amd?void define("strophe-websocket",["strophe-core"],function(core){return factory(core.Strophe,core.$build)}):factory(Strophe,$build)}(this,function(Strophe,$build){return Strophe.Websocket=function(connection){this._conn=connection,this.strip="wrapper";var service=connection.service;if(0!==service.indexOf("ws:")&&0!==service.indexOf("wss:")){var new_service="";new_service+="ws"===connection.options.protocol&&"https:"!==window.location.protocol?"ws":"wss",new_service+="://"+window.location.host,new_service+=0!==service.indexOf("/")?window.location.pathname+service:service,connection.service=new_service}},Strophe.Websocket.prototype={_buildStream:function(){return $build("open",{xmlns:Strophe.NS.FRAMING,to:this._conn.domain,version:"1.0"})},_check_streamerror:function(bodyWrap,connectstatus){var errors;if(errors=bodyWrap.getElementsByTagNameNS?bodyWrap.getElementsByTagNameNS(Strophe.NS.STREAM,"error"):bodyWrap.getElementsByTagName("stream:error"),0===errors.length)return!1;for(var error=errors[0],condition="",text="",ns="urn:ietf:params:xml:ns:xmpp-streams",i=0;i<error.childNodes.length;i++){var e=error.childNodes[i];if(e.getAttribute("xmlns")!==ns)break;"text"===e.nodeName?text=e.textContent:condition=e.nodeName}var errorString="WebSocket stream error: ";return errorString+=condition?condition:"unknown",text&&(errorString+=" - "+condition),Strophe.error(errorString),this._conn._changeConnectStatus(connectstatus,condition),this._conn._doDisconnect(),!0},_reset:function(){},_connect:function(){this._closeSocket(),this.socket=new WebSocket(this._conn.service,"xmpp"),this.socket.onopen=this._onOpen.bind(this),this.socket.onerror=this._onError.bind(this),this.socket.onclose=this._onClose.bind(this),this.socket.onmessage=this._connect_cb_wrapper.bind(this)},_connect_cb:function(bodyWrap){var error=this._check_streamerror(bodyWrap,Strophe.Status.CONNFAIL);return error?Strophe.Status.CONNFAIL:void 0},_handleStreamStart:function(message){var error=!1,ns=message.getAttribute("xmlns");"string"!=typeof ns?error="Missing xmlns in <open />":ns!==Strophe.NS.FRAMING&&(error="Wrong xmlns in <open />: "+ns);var ver=message.getAttribute("version");return"string"!=typeof ver?error="Missing version in <open />":"1.0"!==ver&&(error="Wrong version in <open />: "+ver),error?(this._conn._changeConnectStatus(Strophe.Status.CONNFAIL,error),this._conn._doDisconnect(),!1):!0},_connect_cb_wrapper:function(message){if(0===message.data.indexOf("<open ")||0===message.data.indexOf("<?xml")){var data=message.data.replace(/^(<\?.*?\?>\s*)*/,"");if(""===data)return;var streamStart=(new DOMParser).parseFromString(data,"text/xml").documentElement;this._conn.xmlInput(streamStart),this._conn.rawInput(message.data),this._handleStreamStart(streamStart)&&this._connect_cb(streamStart)}else if(0===message.data.indexOf("<close ")){this._conn.rawInput(message.data),this._conn.xmlInput(message);var see_uri=message.getAttribute("see-other-uri");see_uri?(this._conn._changeConnectStatus(Strophe.Status.REDIRECT,"Received see-other-uri, resetting connection"),this._conn.reset(),this._conn.service=see_uri,this._connect()):(this._conn._changeConnectStatus(Strophe.Status.CONNFAIL,"Received closing stream"),this._conn._doDisconnect())}else{var string=this._streamWrap(message.data),elem=(new DOMParser).parseFromString(string,"text/xml").documentElement;this.socket.onmessage=this._onMessage.bind(this),this._conn._connect_cb(elem,null,message.data)}},_disconnect:function(pres){if(this.socket&&this.socket.readyState!==WebSocket.CLOSED){pres&&this._conn.send(pres);var close=$build("close",{xmlns:Strophe.NS.FRAMING});this._conn.xmlOutput(close);var closeString=Strophe.serialize(close);this._conn.rawOutput(closeString);try{this.socket.send(closeString)}catch(e){Strophe.info("Couldn't send <close /> tag.")}}this._conn._doDisconnect()},_doDisconnect:function(){Strophe.info("WebSockets _doDisconnect was called"),this._closeSocket()},_streamWrap:function(stanza){return"<wrapper>"+stanza+"</wrapper>"},_closeSocket:function(){if(this.socket)try{this.socket.close()}catch(e){}this.socket=null},_emptyQueue:function(){return!0},_onClose:function(){this._conn.connected&&!this._conn.disconnecting?(Strophe.error("Websocket closed unexcectedly"),this._conn._doDisconnect()):Strophe.info("Websocket closed")},_no_auth_received:function(_callback){Strophe.error("Server did not send any auth methods"),this._conn._changeConnectStatus(Strophe.Status.CONNFAIL,"Server did not send any auth methods"),_callback&&(_callback=_callback.bind(this._conn))(),this._conn._doDisconnect()},_onDisconnectTimeout:function(){},_abortAllRequests:function(){},_onError:function(error){Strophe.error("Websocket error "+error),this._conn._changeConnectStatus(Strophe.Status.CONNFAIL,"The WebSocket connection could not be established was disconnected."),this._disconnect()},_onIdle:function(){var data=this._conn._data;if(data.length>0&&!this._conn.paused){for(var i=0;i<data.length;i++)if(null!==data[i]){var stanza,rawStanza;stanza="restart"===data[i]?this._buildStream().tree():data[i],rawStanza=Strophe.serialize(stanza),this._conn.xmlOutput(stanza),this._conn.rawOutput(rawStanza),this.socket.send(rawStanza)}this._conn._data=[]}},_onMessage:function(message){var elem,data,close='<close xmlns="urn:ietf:params:xml:ns:xmpp-framing" />';if(message.data===close)return this._conn.rawInput(close),this._conn.xmlInput(message),void(this._conn.disconnecting||this._conn._doDisconnect());if(0===message.data.search("<open ")){if(elem=(new DOMParser).parseFromString(message.data,"text/xml").documentElement,!this._handleStreamStart(elem))return}else data=this._streamWrap(message.data),elem=(new DOMParser).parseFromString(data,"text/xml").documentElement;return this._check_streamerror(elem,Strophe.Status.ERROR)?void 0:this._conn.disconnecting&&"presence"===elem.firstChild.nodeName&&"unavailable"===elem.firstChild.getAttribute("type")?(this._conn.xmlInput(elem),void this._conn.rawInput(Strophe.serialize(elem))):void this._conn._dataRecv(elem,message.data)},_onOpen:function(){Strophe.info("Websocket open");var start=this._buildStream();this._conn.xmlOutput(start.tree());var startString=Strophe.serialize(start);this._conn.rawOutput(startString),this.socket.send(startString)},_reqToData:function(stanza){return stanza},_send:function(){this._conn.flush()},_sendRestart:function(){clearTimeout(this._conn._idleTimeout),this._conn._onIdle.bind(this._conn)()}},Strophe}),callback?callback(Strophe,$build,$msg,$iq,$pres):void 0}(function(Strophe,build,msg,iq,pres){window.Strophe=Strophe,window.$build=build,window.$msg=msg,window.$iq=iq,window.$pres=pres}),function(window,undefined){if("undefined"==typeof Strophe)throw"need Strophe";var Easemob=Easemob||{};Easemob.im=Easemob.im||{},Easemob.im.version="2.0",window.URL=window.URL||window.webkitURL||window.mozURL||window.msURL;var Utils=function(){var _createStandardXHR=function(){try{return new window.XMLHttpRequest}catch(e){return!1}},_createActiveXHR=function(){try{return new window.ActiveXObject("Microsoft.XMLHTTP")}catch(e){return!1}};return window.XDomainRequest&&(XDomainRequest.prototype.oldsend=XDomainRequest.prototype.send,XDomainRequest.prototype.send=function(){XDomainRequest.prototype.oldsend.apply(this,arguments),this.readyState=2}),Strophe.Request.prototype._newXHR=function(){var xhr=Utils.xmlrequest(!0);return xhr.overrideMimeType&&xhr.overrideMimeType("text/xml"),xhr.onreadystatechange=this.func.bind(null,this),xhr},{hasFormData:"undefined"!=typeof FormData,hasBlob:"undefined"!=typeof Blob,isCanSetRequestHeader:function(){return Utils.xmlrequest().setRequestHeader||!1},hasOverrideMimeType:function(){return Utils.xmlrequest().overrideMimeType||!1},isCanUploadFileAsync:function(){return Utils.isCanSetRequestHeader()&&Utils.hasFormData},isCanUploadFile:function(){return Utils.isCanUploadFileAsync()||Utils.hasFlash},isCanDownLoadFile:function(){return Utils.isCanSetRequestHeader()&&(Utils.hasBlob||Utils.hasOverrideMimeType())},stringify:function(json){if(JSON&&JSON.stringify)return JSON.stringify(json);var s="",arr=[],iterate=function(json){var isArr=!1;"[object Array]"==Object.prototype.toString.call(json)?(arr.push("]","["),isArr=!0):"[object Object]"==Object.prototype.toString.call(json)&&arr.push("}","{");for(var o in json)"[object Null]"==Object.prototype.toString.call(json[o])?json[o]="null":"[object Undefined]"==Object.prototype.toString.call(json[o])&&(json[o]="undefined"),s+=json[o]&&"object"==typeof json[o]?","+(isArr?"":'"'+o+'":'+(isArr?'"':""))+iterate(json[o]):',"'+(isArr?"":o+'":"')+json[o]+'"';return""!=s&&(s=s.slice(1)),arr.pop()+s+arr.pop()};return iterate(json)},registerUserFn:function(options){var orgName=options.orgName||"",appName=options.appName||"",appKey=options.appKey||"";if(!orgName&&!appName&&appKey){var devInfos=appKey.split("#");2==devInfos.length&&(orgName=devInfos[0],appName=devInfos[1])}if(!orgName&&!appName)return void options.error({type:EASEMOB_IM_RESISTERUSER_ERROR,msg:"没有指定开发者信息"});var url=options.https?"https://a1.easemob.com":"http://a1.easemob.com",apiUrl=options.apiUrl||url,restUrl=apiUrl+"/"+orgName+"/"+appName+"/users",userjson={username:options.username,password:options.password,nickname:options.nickname||""},userinfo=Utils.stringify(userjson),options={url:restUrl,dataType:"json",data:userinfo,success:options.success||EMPTYFN,error:options.error||EMPTYFN};return Utils.ajax(options)},login2UserGrid:function(options){options=options||{};var appKey=options.appKey||"",devInfos=appKey.split("#");if(2!=devInfos.length)return error({type:EASEMOB_IM_CONNCTION_OPEN_USERGRID_ERROR,msg:"请指定正确的开发者信息(appKey)"}),!1;var orgName=devInfos[0],appName=devInfos[1];if(!orgName)return error({type:EASEMOB_IM_CONNCTION_OPEN_USERGRID_ERROR,msg:"请指定正确的开发者信息(appKey)"}),!1;if(!appName)return error({type:EASEMOB_IM_CONNCTION_OPEN_USERGRID_ERROR,msg:"请指定正确的开发者信息(appKey)"}),!1;var suc=options.success||EMPTYFN,error=options.error||EMPTYFN,user=options.user||"",pwd=options.pwd||"",https=options.https,url=https?"https://a1.easemob.com":"http://a1.easemob.com",apiUrl=options.apiUrl||url,loginJson={grant_type:"password",username:user,password:pwd},loginfo=Utils.stringify(loginJson),options={url:apiUrl+"/"+orgName+"/"+appName+"/token",dataType:"json",data:loginfo,success:suc||EMPTYFN,error:error||EMPTYFN};return Utils.ajax(options)},getFileUrl:function(fileInputId){var uri={url:"",filename:"",filetype:"",data:""};if(!Utils.isCanUploadFileAsync())return uri;if(window.URL.createObjectURL){var fileItems=document.getElementById(fileInputId).files;if(fileItems.length>0){var u=fileItems.item(0);uri.data=u,uri.url=window.URL.createObjectURL(u),uri.filename=u.name||""}}else{var u=document.getElementById(fileInputId).value;uri.url=u;var pos1=u.lastIndexOf("/"),pos2=u.lastIndexOf("\\"),pos=Math.max(pos1,pos2);0>pos?uri.filename=u:uri.filename=u.substring(pos+1)}var index=uri.filename.lastIndexOf(".");return-1!=index&&(uri.filetype=uri.filename.substring(index+1).toLowerCase()),uri},getFileSizeFn:function(fileInputId){var file=document.getElementById(fileInputId),fileSize=0;if(file)if(file.files)file.files.length>0&&(fileSize=file.files[0].size);else if(file.select&&"ActiveXObject"in window){file.select();var fileobject=new ActiveXObject("Scripting.FileSystemObject"),file=fileobject.GetFile(file.value);fileSize=file.Size}return fileSize},hasFlash:function(){if("ActiveXObject"in window)try{return new ActiveXObject("ShockwaveFlash.ShockwaveFlash")}catch(ex){return 0}else if(navigator.plugins&&navigator.plugins.length>0)return navigator.plugins["Shockwave Flash"];return 0}(),trim:function(str){return str="string"==typeof str?str:"",str.trim?str.trim():str.replace(/^\s|\s$/g,"")},parseLink:function(msg){var reg=new RegExp("(http(s)?://|www[.])[a-zA-Z0-9-]+[.][0-9a-zA-Z]+(([/]?[a-zA-Z0-9-]+[.]?[a-zA-Z0-9]*)+)?([/]?[?]?[_0-9a-zA-Z&=#]+)?","gm"),res=msg.match(reg),src=res&&res[0]?res[0]:"";if(res&&res.length){var prefix=/^https?:\/\//.test(src);msg=msg.replace(reg,"<a href='"+(prefix?src:"//"+src)+"' target='_blank'>"+src+"</a>")}return msg},parseJSON:function(data){if(window.JSON&&window.JSON.parse)return window.JSON.parse(data+"");var requireNonComma,depth=null,str=Utils.trim(data+"");return str&&!Utils.trim(str.replace(/(,)|(\[|{)|(}|])|"(?:[^"\\\r\n]|\\["\\\/bfnrt]|\\u[\da-fA-F]{4})*"\s*:?|true|false|null|-?(?!0\d)\d+(?:\.\d+|)(?:[eE][+-]?\d+|)/g,function(token,comma,open,close){return requireNonComma&&comma&&(depth=0),0===depth?token:(requireNonComma=open||comma,depth+=!close-!open,"")}))?Function("return "+str)():Function("Invalid JSON: "+data)()},parseUploadResponse:function(response){return response.indexOf("callback")>-1?response.slice(9,-1):response},parseDownloadResponse:function(response){return response&&response.type&&"application/json"===response.type||0>Object.prototype.toString.call(response).indexOf("Blob")?this.url+"?token=":window.URL.createObjectURL(response)},uploadFile:function(options){options=options||{},options.onFileUploadProgress=options.onFileUploadProgress||EMPTYFN,options.onFileUploadComplete=options.onFileUploadComplete||EMPTYFN,options.onFileUploadError=options.onFileUploadError||EMPTYFN,options.onFileUploadCanceled=options.onFileUploadCanceled||EMPTYFN;var acc=options.accessToken||this.context.accessToken;if(!acc)return void options.onFileUploadError({type:EASEMOB_IM_UPLOADFILE_NO_LOGIN,msg:"用户未登录到usergrid服务器,无法使用文件上传功能",id:options.id});if(orgName=options.orgName||this.context.orgName||"",appName=options.appName||this.context.appName||"",appKey=options.appKey||this.context.appKey||"",!orgName&&!appName&&appKey){var devInfos=appKey.split("#");2==devInfos.length&&(orgName=devInfos[0],appName=devInfos[1])}if(!orgName&&!appName)return void options.onFileUploadError({type:EASEMOB_IM_UPLOADFILE_ERROR,msg:"没有指定开发者信息",id:options.id});var apiUrl=options.apiUrl||"http://a1.easemob.com",uploadUrl=apiUrl+"/"+orgName+"/"+appName+"/chatfiles";if(!Utils.isCanUploadFileAsync())return void(Utils.hasFlash&&"function"==typeof options.flashUpload?options.flashUpload&&options.flashUpload(uploadUrl,options):this.onError({type:EASEMOB_IM_UPLOADFILE_BROWSER_ERROR,msg:"当前浏览器不支持异步上传！"}));var fileSize=options.file.data?options.file.data.size:undefined;if(fileSize>EASEMOB_IM_FILESIZE_LIMIT)return void options.onFileUploadError({type:EASEMOB_IM_UPLOADFILE_ERROR,msg:"上传文件超过服务器大小限制（10M）",id:options.id});if(0>=fileSize)return void options.onFileUploadError({type:EASEMOB_IM_UPLOADFILE_ERROR,msg:"上传文件大小为0",id:options.id});var xhr=Utils.xmlrequest(),onError=function(e){options.onFileUploadError({type:EASEMOB_IM_UPLOADFILE_ERROR,msg:"上传文件失败",id:options.id,xhr:xhr})};xhr.upload&&xhr.upload.addEventListener("progress",options.onFileUploadProgress,!1),xhr.addEventListener?(xhr.addEventListener("abort",options.onFileUploadCanceled,!1),xhr.addEventListener("load",function(e){try{var json=Utils.parseJSON(xhr.responseText);options.onFileUploadComplete(json)}catch(e){options.onFileUploadError({type:EASEMOB_IM_UPLOADFILE_ERROR,msg:"上传文件失败,服务端返回值值不正确",data:xhr.responseText,id:options.id,xhr:xhr})}},!1),xhr.addEventListener("error",onError,!1)):xhr.onreadystatechange&&(xhr.onreadystatechange=function(){if(4===xhr.readyState)if(200==ajax.status)try{var json=Utils.parseJSON(xhr.responseText);options.onFileUploadComplete(json)}catch(e){options.onFileUploadError({type:EASEMOB_IM_UPLOADFILE_ERROR,msg:"上传文件失败,服务端返回值不正确",data:xhr.responseText,id:options.id,xhr:xhr})}else options.onFileUploadError({type:EASEMOB_IM_UPLOADFILE_ERROR,msg:"上传文件失败,服务端返回异常",data:xhr.responseText,id:options.id,xhr:xhr});else xhr.abort(),options.onFileUploadCanceled()}),xhr.open("POST",uploadUrl),xhr.setRequestHeader("restrict-access","true"),xhr.setRequestHeader("Accept","*/*"),xhr.setRequestHeader("Authorization","Bearer "+acc);var formData=new FormData;formData.append("file",options.file.data),xhr.send(formData)},downloadFn:function(options){options.onFileDownloadComplete=options.onFileDownloadComplete||EMPTYFN,options.onFileDownloadError=options.onFileDownloadError||EMPTYFN;var accessToken=options.accessToken||"";if(!accessToken)return void options.onFileDownloadError({type:EASEMOB_IM_DOWNLOADFILE_NO_LOGIN,msg:"用户未登录到usergrid服务器,无法使用文件下载功能",id:options.id});var onError=function(e){options.onFileDownloadError({type:EASEMOB_IM_DOWNLOADFILE_ERROR,msg:"下载文件失败",id:options.id,xhr:xhr})};if(!Utils.isCanDownLoadFile())return void options.onFileDownloadComplete();var xhr=Utils.xmlrequest();"addEventListener"in xhr?(xhr.addEventListener("load",function(e){options.onFileDownloadComplete(xhr.response,xhr)},!1),xhr.addEventListener("error",onError,!1)):"onreadystatechange"in xhr&&(xhr.onreadystatechange=function(){4===xhr.readyState?200==ajax.status?options.onFileDownloadComplete(xhr.response,xhr):options.onFileDownloadError({type:EASEMOB_IM_DOWNLOADFILE_ERROR,msg:"下载文件失败,服务端返回异常",id:options.id,xhr:xhr}):(xhr.abort(),options.onFileDownloadError({type:EASEMOB_IM_DOWNLOADFILE_ERROR,msg:"错误的下载状态,退出下载",id:options.id,xhr:xhr}))});var method=options.method||"GET",resType=options.responseType||"blob",mimeType=options.mimeType||"text/plain; charset=x-user-defined";xhr.open(method,options.url),"undefined"!=typeof Blob?xhr.responseType=resType:xhr.overrideMimeType(mimeType);var innerHeaer={"X-Requested-With":"XMLHttpRequest",Accept:"application/octet-stream","share-secret":options.secret,Authorization:"Bearer "+accessToken},headers=options.headers||{};for(var key in headers)innerHeaer[key]=headers[key];for(var key in innerHeaer)innerHeaer[key]&&xhr.setRequestHeader(key,innerHeaer[key]);xhr.send(null)},parseTextMessage:function(message,faces){if("string"!=typeof message)return void conn.onError({type:EASEMOB_IM_MESSAGE_REC_TEXT_ERROR,msg:"不合法的消息内容格式，请检查发送消息内容！"});if("[object Object]"!=Object.prototype.toString.call(faces))return{isemotion:!1,body:[{type:"txt",data:message}]};var receiveMsg=message,emessage=[],expr=/\[[^[\]]{2,3}\]/gm,emotions=receiveMsg.match(expr);if(!emotions||emotions.length<1)return{isemotion:!1,body:[{type:"txt",data:message}]};for(var isemotion=!1,i=0;i<emotions.length;i++){var tmsg=receiveMsg.substring(0,receiveMsg.indexOf(emotions[i]));tmsg&&emessage.push({type:"txt",data:tmsg});var emotion=Easemob.im.EMOTIONS[emotions[i]];emotion?(isemotion=!0,emessage.push({type:"emotion",data:emotion})):emessage.push({type:"txt",data:emotions[i]});var restMsgIndex=receiveMsg.indexOf(emotions[i])+emotions[i].length;receiveMsg=receiveMsg.substring(restMsgIndex)}return receiveMsg&&emessage.push({type:"txt",data:receiveMsg}),isemotion?{isemotion:isemotion,body:emessage}:{isemotion:!1,body:[{type:"txt",data:message}]}},xmlrequest:function(crossDomain){crossDomain=crossDomain||!0;var temp=_createStandardXHR()||_createActiveXHR();if("withCredentials"in temp)return temp;if(!crossDomain)return temp;if(window.XDomainRequest===undefined)return temp;var xhr=new XDomainRequest;return xhr.readyState=0,xhr.status=100,xhr.onreadystatechange=EMPTYFN,xhr.onload=function(){xhr.readyState=4,xhr.status=200;var xmlDoc=new ActiveXObject("Microsoft.XMLDOM");xmlDoc.async="false",xmlDoc.loadXML(xhr.responseText),xhr.responseXML=xmlDoc,xhr.response=xhr.responseText,xhr.onreadystatechange()},xhr.ontimeout=xhr.onerror=function(){xhr.readyState=4,xhr.status=500,xhr.onreadystatechange()},xhr},ajax:function(options){var dataType=options.dataType||"text",suc=options.success||EMPTYFN,error=options.error||EMPTYFN,xhr=Utils.xmlrequest();if(xhr.onreadystatechange=function(){if(4===xhr.readyState){var status=xhr.status||0;if(200==status){if("text"==dataType)return void suc(xhr.responseText,xhr);if("json"==dataType){try{var json=Utils.parseJSON(xhr.responseText);suc(json,xhr)}catch(e){error(xhr.responseText,xhr,"错误的数据,无法转换为json")}return}return"xml"==dataType?void(xhr.responseXML&&xhr.responseXML.documentElement?suc(xhr.responseXML.documentElement,xhr):error(xhr.responseText,xhr,"浏览器不支持ajax返回xml对象")):void suc(xhr.response||xhr.responseText,xhr)}if("json"==dataType){try{var json=Utils.parseJSON(xhr.responseText);error(json,xhr,"服务器返回错误信息")}catch(e){error(xhr.responseText,xhr,"服务器返回错误信息")}return}return"xml"==dataType?void(xhr.responseXML&&xhr.responseXML.documentElement?error(xhr.responseXML.documentElement,xhr,"服务器返回错误信息"):error(xhr.responseText,xhr,"服务器返回错误信息")):void error(xhr.responseText,xhr,"服务器返回错误信息")}0===xhr.readyState&&error(xhr.responseText,xhr,"服务器异常")},options.responseType){if(!xhr.responseType)return error("",xhr,"当前浏览器不支持设置响应类型"),null;xhr.responseType=options.responseType}if(options.mimeType){if(!Utils.hasOverrideMimeType())return error("",xhr,"当前浏览器不支持设置mimeType"),null;xhr.overrideMimeType(options.mimeType)}var type=options.type||"POST";xhr.open(type,options.url);var headers=options.headers||{};for(var key in headers){if(!Utils.isCanSetRequestHeader())return error("",xhr,"当前浏览器不支持设置header"),null;xhr.setRequestHeader(key,headers[key])}var data=options.data||null;return xhr.send(data),xhr}}}(),_msgHash={},Message=function(message){return this instanceof Message?void(this.msg=message):new Message(message,conn)};Message.prototype.send=function(conn){var me=this,_send=function(message){var json={from:conn.context.userId||"",to:message.to,bodies:[message.body],ext:message.ext||{}},jsonstr=Utils.stringify(json),dom=$msg({type:message.type||"chat",to:message.toJid,id:message.id,xmlns:"jabber:client"}).c("body").t(jsonstr);conn.isOpened()?me.st&&clearTimeout(me.st):me.st=setTimeout(function(){_msgHash[message.id]&&("undefined"==typeof _msgHash[message.id].timeout&&(_msgHash[message.id].timeout=4),0==_msgHash[message.id].timeout?(_msgHash[message.id].timeout=4,_msgHash[message.id].msg.fail instanceof Function&&_msgHash[message.id].msg.fail(message.id)):(_msgHash[message.id].timeout-=1,_send(message)))},1e4);try{conn.sendCommand(dom.tree())}catch(e){}};if(me.msg.file){if(me.msg.body&&me.msg.body.url)return void _send(me.msg);var _tmpComplete=me.msg.onFileUploadComplete,_complete=function(data){if(data.entities[0]["file-metadata"]){var file_len=data.entities[0]["file-metadata"]["content-length"];me.msg.file_length=file_len,me.msg.filetype=data.entities[0]["file-metadata"]["content-type"],file_len>204800&&(me.msg.thumbnail=!0)}me.msg.body={type:me.msg.ext.messageType||"file",url:data.uri+"/"+data.entities[0].uuid,secret:data.entities[0]["share-secret"],filename:me.msg.file.filename,thumb:data.uri+"/"+data.entities[0].uuid,thumb_secret:"",size:{width:me.msg.width,height:me.msg.height},file_length:me.msg.file_length,filetype:me.msg.filetype},_send(me.msg),_tmpComplete instanceof Function&&_tmpComplete(data,me.msg.id)};me.msg.onFileUploadComplete=_complete,Utils.uploadFile.call(conn,me.msg)}else me.msg.body={type:"txt",msg:me.msg.msg},_send(me.msg)};var Connection=function(){var _parseRoomFn=function(result){var rooms=[],items=result.getElementsByTagName("item");if(items)for(var i=0;i<items.length;i++){var item=items[i],roomJid=item.getAttribute("jid"),tmp=roomJid.split("@")[0],room={jid:roomJid,name:item.getAttribute("name"),roomId:tmp.split("_")[1]};rooms.push(room)}return rooms},_parseRoomOccupantsFn=function(result){var occupants=[],items=result.getElementsByTagName("item");if(items)for(var i=0;i<items.length;i++){var item=items[i],room={jid:item.getAttribute("jid"),name:item.getAttribute("name")};occupants.push(room)}return occupants},_parseResponseMessage=function(msginfo){var parseMsgData={errorMsg:!0,data:[]},msgBodies=msginfo.getElementsByTagName("body");if(msgBodies){for(var i=0;i<msgBodies.length;i++){var msgBody=msgBodies[i],childNodes=msgBody.childNodes;if(childNodes&&childNodes.length>0){var childNode=msgBody.childNodes[0];if(childNode.nodeType==Strophe.ElementType.TEXT){var jsondata=childNode.wholeText||childNode.nodeValue;jsondata=jsondata.replace("\n","<br>");try{var data=eval("("+jsondata+")");parseMsgData.errorMsg=!1,parseMsgData.data=[data]}catch(e){}}}}var delayTags=msginfo.getElementsByTagName("delay");if(delayTags&&delayTags.length>0){var delayTag=delayTags[0],delayMsgTime=delayTag.getAttribute("stamp");delayMsgTime&&(parseMsgData.delayTimeStamp=delayMsgTime)}}else{var childrens=msginfo.childNodes;if(childrens&&childrens.length>0){var child=msginfo.childNodes[0];if(child.nodeType==Strophe.ElementType.TEXT)try{var data=eval("("+child.nodeValue+")");parseMsgData.errorMsg=!1,parseMsgData.data=[data]}catch(e){}}}return parseMsgData},_parseNameFromJidFn=function(jid,domain){domain=domain||"";var tempstr=jid,findex=tempstr.indexOf("_");-1!=findex&&(tempstr=tempstr.substring(findex+1));var atindex=tempstr.indexOf("@"+domain);return-1!=atindex&&(tempstr=tempstr.substring(0,atindex)),tempstr},_parseFriendFn=function(queryTag){var rouster=[],items=queryTag.getElementsByTagName("item");if(items)for(var i=0;i<items.length;i++){var item=items[i],jid=item.getAttribute("jid");if(jid){var subscription=item.getAttribute("subscription"),friend={subscription:subscription,jid:jid},ask=item.getAttribute("ask");ask&&(friend.ask=ask);var name=item.getAttribute("name");if(name)friend.name=name;else{var n=_parseNameFromJidFn(jid);friend.name=n}var groups=[];Strophe.forEachChild(item,"group",function(group){groups.push(Strophe.getText(group))}),friend.groups=groups,rouster.push(friend)}}return rouster},_dologin2IM=function(options,conn){var accessToken=options.access_token||"";if(""==accessToken){var loginfo=Utils.stringify(options);return void conn.onError({type:EASEMOB_IM_CONNCTION_OPEN_USERGRID_ERROR,msg:"登录失败,"+loginfo,data:options,xhr:xhr})}conn.context.accessToken=options.access_token,conn.context.accessTokenExpires=options.expires_in;var stropheConn=conn.context.stropheConn||new Strophe.Connection(conn.url,{inactivity:conn.inactivity,maxRetries:conn.maxRetries,pollingTime:conn.pollingTime}),callback=function(status,msg){_login2ImCallback(status,msg,conn)},jid=conn.context.jid;conn.context.stropheConn=stropheConn,conn.route?stropheConn.connect(jid,"$t$"+accessToken,callback,conn.wait,conn.hold,conn.route):stropheConn.connect(jid,"$t$"+accessToken,callback,conn.wait,conn.hold)},_parseMessageType=function(msginfo){var msgtype="normal",receiveinfo=msginfo.getElementsByTagName("received");if(receiveinfo&&receiveinfo.length>0&&"urn:xmpp:receipts"==receiveinfo[0].namespaceURI)msgtype="received";else{var inviteinfo=msginfo.getElementsByTagName("invite");inviteinfo&&inviteinfo.length>0&&(msgtype="invite")}return msgtype},_login2ImCallback=function(status,msg,conn){if(status==Strophe.Status.CONNFAIL)conn.onError({type:EASEMOB_IM_CONNCTION_SERVER_CLOSE_ERROR,msg:msg
});else if(status==Strophe.Status.ATTACHED||status==Strophe.Status.CONNECTED){var handleMessage=function(msginfo){var type=_parseMessageType(msginfo);return"received"==type?(conn.handleReceivedMessage(msginfo),!0):"invite"==type?(conn.handleInviteMessage(msginfo),!0):(conn.handleMessage(msginfo),!0)},handlePresence=function(msginfo){return conn.handlePresence(msginfo),!0},handlePing=function(msginfo){return conn.handlePing(msginfo),!0},handleIq=function(msginfo){return conn.handleIq(msginfo),!0};conn.addHandler(handleMessage,null,"message",null,null,null),conn.addHandler(handlePresence,null,"presence",null,null,null),conn.addHandler(handlePing,"urn:xmpp:ping","iq","get",null,null),conn.addHandler(handleIq,"jabber:iq:roster","iq","set",null,null),conn.context.status=STATUS_OPENED;var supportRecMessage=[EASEMOB_IM_MESSAGE_REC_TEXT,EASEMOB_IM_MESSAGE_REC_EMOTION];Utils.isCanDownLoadFile()&&(supportRecMessage.push(EASEMOB_IM_MESSAGE_REC_PHOTO),supportRecMessage.push(EASEMOB_IM_MESSAGE_REC_AUDIO_FILE));var supportSedMessage=[EASEMOB_IM_MESSAGE_SED_TEXT];Utils.isCanUploadFile()&&(supportSedMessage.push(EASEMOB_IM_MESSAGE_REC_PHOTO),supportSedMessage.push(EASEMOB_IM_MESSAGE_REC_AUDIO_FILE)),conn.notifyVersion(),conn.onOpened({canReceive:supportRecMessage,canSend:supportSedMessage,accessToken:conn.context.accessToken})}else status==Strophe.Status.DISCONNECTING?conn.isOpened()&&(conn.context.status=STATUS_CLOSING,conn.onError({type:EASEMOB_IM_CONNCTION_SERVER_CLOSE_ERROR,msg:msg})):status==Strophe.Status.DISCONNECTED?(conn.context.status=STATUS_CLOSED,conn.clear(),conn.onClosed()):status==Strophe.Status.AUTHFAIL?(conn.onError({type:EASEMOB_IM_CONNCTION_AUTH_ERROR,msg:"登录失败,请输入正确的用户名和密码"}),conn.clear()):status==Strophe.Status.ERROR&&conn.onError({type:EASEMOB_IM_CONNCTION_SERVER_ERROR,msg:msg||"服务器异常"})},_getJid=function(options,conn){var jid=options.toJid||"";if(""==jid){var appKey=conn.context.appKey||"",toJid=appKey+"_"+options.to+"@"+conn.domain;options.resource&&(toJid=toJid+"/"+options.resource),jid=toJid}return jid},_innerCheck=function(options,conn){if(conn.isOpened()||conn.isOpening())return conn.onError({type:EASEMOB_IM_CONNCTION_REOPEN_ERROR,msg:"重复打开连接,请先关闭连接再打开"}),!1;options=options||{};var user=options.user||"";if(""==options.user)return conn.onError({type:EASEMOB_IM_CONNCTION_USER_NOT_ASSIGN_ERROR,msg:"未指定用户"}),!1;var appKey=options.appKey||"",devInfos=appKey.split("#");if(2!=devInfos.length)return conn.onError({type:EASEMOB_IM_CONNCTION_OPEN_ERROR,msg:"请指定正确的开发者信息(appKey)"}),!1;var orgName=devInfos[0],appName=devInfos[1];if(!orgName)return conn.onError({type:EASEMOB_IM_CONNCTION_OPEN_ERROR,msg:"请指定正确的开发者信息(appKey)"}),!1;if(!appName)return conn.onError({type:EASEMOB_IM_CONNCTION_OPEN_ERROR,msg:"请指定正确的开发者信息(appKey)"}),!1;var jid=appKey+"_"+user+"@"+conn.domain,resource_value="webim",resource=options.resource||resource_value;return""!=resource&&(jid=jid+"/"+resource),conn.context.jid=jid,conn.context.userId=user,conn.context.appKey=appKey,conn.context.appName=appName,conn.context.orgName=orgName,!0},connection=function(options){if(!(this instanceof Connection))return new Connection(options);var _prefix;window.WebSocket?(_prefix=options.wss?"wss":"ws",this.url=options.url||_prefix+"://im-api.easemob.com/ws/"):(_prefix=options.https?"https":"http",this.url=(options.url&&options.url.indexOf("ws:")>-1?"":options.url)||_prefix+"://im-api.easemob.com/http-bind/"),this.https=options.https||!1,this.wait=options.wait||30,this.hold=options.hold||1,options.route&&(this.route=options.route),this.domain=options.domain||"easemob.com",this.inactivity=options.inactivity||30,this.maxRetries=options.maxRetries||5,this.pollingTime=options.pollingTime||800,this.stropheConn=!1,this.context={status:STATUS_INIT}};return connection.prototype.listen=function(options){this.onOpened=options.onOpened||EMPTYFN,this.onClosed=options.onClosed||EMPTYFN,this.onTextMessage=options.onTextMessage||EMPTYFN,this.onEmotionMessage=options.onEmotionMessage||EMPTYFN,this.onPictureMessage=options.onPictureMessage||EMPTYFN,this.onAudioMessage=options.onAudioMessage||EMPTYFN,this.onVideoMessage=options.onVideoMessage||EMPTYFN,this.onFileMessage=options.onFileMessage||EMPTYFN,this.onLocationMessage=options.onLocationMessage||EMPTYFN,this.onCmdMessage=options.onCmdMessage||EMPTYFN,this.onPresence=options.onPresence||EMPTYFN,this.onRoster=options.onRoster||EMPTYFN,this.onError=options.onError||EMPTYFN,this.onReceivedMessage=options.onReceivedMessage||EMPTYFN,this.onInviteMessage=options.onInviteMessage||EMPTYFN},connection.prototype.sendReceiptsMessage=function(options){var dom=$msg({from:this.context.jid||"",to:"easemob.com",id:options.id||""}).c("received",{xmlns:"urn:xmpp:receipts",id:options.id||""});this.sendCommand(dom.tree())},connection.prototype.open=function(options){var pass=_innerCheck(options,this);if(0!=pass){var conn=this;if(options.accessToken)options.access_token=options.accessToken,_dologin2IM(options,conn);else{var loginUrl=this.https?"https://a1.easemob.com":"http://a1.easemob.com",apiUrl=options.apiUrl||loginUrl,userId=this.context.userId,pwd=options.pwd||"",appName=this.context.appName,orgName=this.context.orgName,suc=function(data,xhr){conn.context.status=STATUS_DOLOGIN_IM,_dologin2IM(data,conn)},error=function(res,xhr,msg){res.error&&res.error_description?conn.onError({type:EASEMOB_IM_CONNCTION_OPEN_USERGRID_ERROR,msg:"登录失败,"+res.error_description,data:res,xhr:xhr}):conn.onError({type:EASEMOB_IM_CONNCTION_OPEN_USERGRID_ERROR,msg:"登录失败",data:res,xhr:xhr}),conn.clear()};this.context.status=STATUS_DOLOGIN_USERGRID;var loginJson={grant_type:"password",username:userId,password:pwd},loginfo=Utils.stringify(loginJson),options={url:apiUrl+"/"+orgName+"/"+appName+"/token",dataType:"json",data:loginfo,success:suc||EMPTYFN,error:error||EMPTYFN};Utils.ajax(options)}}},connection.prototype.attach=function(options){var pass=_innerCheck(options,this);if(0!=pass){options=options||{};var accessToken=options.accessToken||"";if(""==accessToken)return void this.onError({type:EASEMOB_IM_CONNCTION_ATTACH_USERGRID_ERROR,msg:"未指定用户的accessToken"});var sid=options.sid||"";if(""==sid)return void this.onError({type:EASEMOB_IM_CONNCTION_ATTACH_ERROR,msg:"未指定用户的会话信息"});var rid=options.rid||"";if(""==rid)return void this.onError({type:EASEMOB_IM_CONNCTION_ATTACH_ERROR,msg:"未指定用户的消息id"});var stropheConn=new Strophe.Connection(this.url,{inactivity:this.inactivity,maxRetries:this.maxRetries,pollingTime:this.pollingTime});this.context.accessToken=accessToken,this.context.stropheConn=stropheConn,this.context.status=STATUS_DOLOGIN_IM;var conn=this,callback=function(status,msg){_login2ImCallback(status,msg,conn)},jid=this.context.jid,wait=this.wait,hold=this.hold,wind=this.wind||5;stropheConn.attach(jid,sid,rid,callback,wait,hold,wind)}},connection.prototype.close=function(){var status=this.context.status;status!=STATUS_INIT&&(this.isClosed()||this.isClosing()||(this.context.status=STATUS_CLOSING,this.context.stropheConn.disconnect()))},connection.prototype.addHandler=function(handler,ns,name,type,id,from,options){this.context.stropheConn.addHandler(handler,ns,name,type,id,from,options)},connection.prototype.notifyVersion=function(suc,fail){var dom=(_getJid({},this),$iq({from:this.context.jid||"",to:this.domain,type:"result"}).c("query",{xmlns:"jabber:iq:version"}).c("name").t("easemob").up().c("version").t(Easemob.im.version).up().c("os").t("webim"));suc=suc||EMPTYFN,error=fail||this.onError;var failFn=function(ele){error({type:EASEMOB_IM_CONNCTION_NOTIFYVERSION_ERROR,msg:"发送版本信息给服务器时失败",data:ele})};this.context.stropheConn.sendIQ(dom.tree(),suc,failFn)},connection.prototype.handlePresence=function(msginfo){if(!this.isClosed()){var from=msginfo.getAttribute("from")||"",to=msginfo.getAttribute("to")||"",type=msginfo.getAttribute("type")||"",fromUser=_parseNameFromJidFn(from),toUser=_parseNameFromJidFn(to),info={from:fromUser,to:toUser,fromJid:from,toJid:to,type:type},showTags=msginfo.getElementsByTagName("show");if(showTags&&showTags.length>0){var showTag=showTags[0];info.show=Strophe.getText(showTag)}var statusTags=msginfo.getElementsByTagName("status");if(statusTags&&statusTags.length>0){var statusTag=statusTags[0];info.status=Strophe.getText(statusTag)}var priorityTags=msginfo.getElementsByTagName("priority");if(priorityTags&&priorityTags.length>0){var priorityTag=priorityTags[0];info.priority=Strophe.getText(priorityTag)}this.onPresence(info,msginfo)}},connection.prototype.handlePing=function(e){if(!this.isClosed()){var id=e.getAttribute("id"),from=e.getAttribute("from"),to=e.getAttribute("to"),dom=$iq({from:to,to:from,id:id,type:"result"});this.sendCommand(dom.tree())}},connection.prototype.handleIq=function(e){var id=e.getAttribute("id"),from=e.getAttribute("from")||"",name=_parseNameFromJidFn(from),curJid=this.context.jid,curUser=this.context.userId;if(""!==from&&from!=curJid&&curUser!=name)return!0;var iqresult=$iq({type:"result",id:id,from:curJid});this.sendCommand(iqresult.tree());var msgBodies=e.getElementsByTagName("query");if(msgBodies&&msgBodies.length>0){var queryTag=msgBodies[0],rouster=_parseFriendFn(queryTag);this.onRoster(rouster)}return!0},connection.prototype.handleMessage=function(msginfo){if(!this.isClosed()){var id=msginfo.getAttribute("id")||"";this.sendReceiptsMessage({id:id});var parseMsgData=_parseResponseMessage(msginfo);if(!parseMsgData.errorMsg){var msgDatas=parseMsgData.data;for(var i in msgDatas){var msg=msgDatas[i],from=msg.from,too=msg.to,extmsg=msg.ext||{},chattype=msginfo.getAttribute("type")||"chat",msgBodies=msg.bodies;if(msgBodies&&0!=msgBodies.length){var msgBody=msg.bodies[0],type=msgBody.type;if("txt"==type){var receiveMsg=msgBody.msg,emotionsbody=Utils.parseTextMessage(receiveMsg,Easemob.im.EMOTION);emotionsbody.isemotion?this.onEmotionMessage({id:id,type:chattype,from:from,to:too,data:emotionsbody.body,ext:extmsg}):this.onTextMessage({id:id,type:chattype,from:from,to:too,data:receiveMsg,ext:extmsg})}else if("img"==type){var rwidth=0,rheight=0;msgBody.size&&(rwidth=msgBody.size.width,rheight=msgBody.size.height);var msg={id:id,type:chattype,from:from,to:too,url:msgBody.url,secret:msgBody.secret,filename:msgBody.filename,thumb:msgBody.thumb,thumb_secret:msgBody.thumb_secret,file_length:msgBody.file_length||"",width:rwidth,height:rheight,filetype:msgBody.filetype||"",accessToken:this.context.accessToken||"",ext:extmsg};this.onPictureMessage(msg)}else"audio"==type?this.onAudioMessage({id:id,type:chattype,from:from,to:too,url:msgBody.url,secret:msgBody.secret,filename:msgBody.filename,length:msgBody.length||"",file_length:msgBody.file_length||"",filetype:msgBody.filetype||"",accessToken:this.context.accessToken||"",ext:extmsg}):"file"==type?this.onFileMessage({id:id,type:chattype,from:from,to:too,url:msgBody.url,secret:msgBody.secret,filename:msgBody.filename,file_length:msgBody.file_length,accessToken:this.context.accessToken||"",ext:extmsg}):"loc"==type?this.onLocationMessage({id:id,type:chattype,from:from,to:too,addr:msgBody.addr,lat:msgBody.lat,lng:msgBody.lng,ext:extmsg}):"video"==type?this.onVideoMessage({id:id,type:chattype,from:from,to:too,url:msgBody.url,secret:msgBody.secret,filename:msgBody.filename,file_length:msgBody.file_length,accessToken:this.context.accessToken||"",ext:extmsg}):"cmd"==type&&this.onCmdMessage({id:id,from:from,to:too,action:msgBody.action,ext:extmsg})}}}}},connection.prototype.handleReceivedMessage=function(message){this.onReceivedMessage(message);var rcv=message.getElementsByTagName("received"),id=undefined;rcv.length>0&&(id=rcv[0].childNodes&&rcv[0].childNodes.length>0?rcv[0].childNodes[0].nodeValue:rcv[0].innerHTML||rcv[0].innerText),_msgHash[id]&&(_msgHash[id].msg.success instanceof Function&&_msgHash[id].msg.success(id),delete _msgHash[id])},connection.prototype.handleInviteMessage=function(message){var form=null,invitemsg=message.getElementsByTagName("invite");if(invitemsg&&invitemsg.length>0){var fromJid=invitemsg[0].getAttribute("from");form=_parseNameFromJidFn(fromJid)}var xmsg=message.getElementsByTagName("x"),roomid=null;if(xmsg&&xmsg.length>0)for(var i=0;i<xmsg.length;i++)if("jabber:x:conference"==xmsg[i].namespaceURI){var roomjid=xmsg[i].getAttribute("jid");roomid=_parseNameFromJidFn(roomjid)}this.onInviteMessage({type:"invite",from:form,roomid:roomid})},connection.prototype.sendCommand=function(dom){this.isOpened()?this.context.stropheConn.send(dom):this.onError({type:EASEMOB_IM_CONNCTION_OPEN_ERROR,msg:"连接还未建立,请先登录或等待登录处理完毕"})},connection.prototype.getUniqueId=function(prefix){var cdate=new Date,offdate=new Date(2010,1,1),offset=cdate.getTime()-offdate.getTime(),hexd=parseInt(offset).toString(16);return"string"==typeof prefix||"number"==typeof prefix?prefix+"_"+hexd:"WEBIM_"+hexd},connection.prototype.send=function(message){if("[object Object]"==Object.prototype.toString.call(message)){var appKey=this.context.appKey||"",toJid=appKey+"_"+message.to+"@"+this.domain;message.type&&"groupchat"==message.type&&(toJid=appKey+"_"+message.to+"@conference."+this.domain),message.resource&&(toJid=toJid+"/"+message.resource),message.toJid=toJid,message.id=message.id||this.getUniqueId(),_msgHash[message.id]=new Message(message),_msgHash[message.id].send(this)}else"string"==typeof message&&_msgHash[message]&&_msgHash[message].send(this)},connection.prototype.heartBeat=function(conn){var options={to:conn.domain,type:"normal"};clearTimeout(conn.heartBeatID),conn.heartBeatID=setTimeout(function(){conn.sendHeartBeatMessage(options)},6e4)},connection.prototype.sendHeartBeatMessage=function(options){var json={},jsonstr=Utils.stringify(json),dom=$msg({to:options.to,type:options.type,id:this.getUniqueId(),xmlns:"jabber:client"}).c("body").t(jsonstr);this.sendCommand(dom.tree())},connection.prototype.stopHeartBeat=function(conn){clearTimeout(conn.heartBeatID)},connection.prototype.addRoster=function(options){var jid=_getJid(options,this),name=options.name||"",groups=options.groups||"",iq=$iq({type:"set"});if(iq.c("query",{xmlns:"jabber:iq:roster"}),iq.c("item",{jid:jid,name:name}),groups)for(var i=0;i<groups.length;i++)iq.c("group").t(groups[i]).up();var suc=options.success||EMPTYFN,error=options.error||EMPTYFN;this.context.stropheConn.sendIQ(iq.tree(),suc,error)},connection.prototype.removeRoster=function(options){var jid=_getJid(options,this),iq=$iq({type:"set"}).c("query",{xmlns:"jabber:iq:roster"}).c("item",{jid:jid,subscription:"remove"}),suc=options.success||EMPTYFN,error=options.error||EMPTYFN;this.context.stropheConn.sendIQ(iq,suc,error)},connection.prototype.getRoster=function(options){var dom=$iq({type:"get"}).c("query",{xmlns:"jabber:iq:roster"});options=options||{},suc=options.success||this.onRoster;var completeFn=function(ele){var rouster=[],msgBodies=ele.getElementsByTagName("query");if(msgBodies&&msgBodies.length>0){var queryTag=msgBodies[0];rouster=_parseFriendFn(queryTag)}suc(rouster,ele)};error=options.error||this.onError;var failFn=function(ele){error({type:EASEMOB_IM_CONNCTION_GETROSTER_ERROR,msg:"获取联系人信息失败",data:ele})};this.isOpened()?this.context.stropheConn.sendIQ(dom.tree(),completeFn,failFn):error({type:EASEMOB_IM_CONNCTION_OPEN_ERROR,msg:"连接还未建立,请先登录或等待登录处理完毕"})},connection.prototype.subscribe=function(options){var jid=_getJid(options,this),pres=$pres({to:jid,type:"subscribe"});options.message&&pres.c("status").t(options.message).up(),options.nick&&pres.c("nick",{xmlns:"http://jabber.org/protocol/nick"}).t(options.nick),this.sendCommand(pres.tree())},connection.prototype.subscribed=function(options){var jid=_getJid(options,this),pres=$pres({to:jid,type:"subscribed"});options.message&&pres.c("status").t(options.message).up(),this.sendCommand(pres.tree())},connection.prototype.unsubscribe=function(options){var jid=_getJid(options,this),pres=$pres({to:jid,type:"unsubscribe"});options.message&&pres.c("status").t(options.message),this.sendCommand(pres.tree())},connection.prototype.unsubscribed=function(options){var jid=_getJid(options,this),pres=$pres({to:jid,type:"unsubscribed"});options.message&&pres.c("status").t(options.message).up(),this.sendCommand(pres.tree())},connection.prototype.createRoom=function(options){var roomiq,suc=options.success||EMPTYFN,err=options.error||EMPTYFN;return roomiq=$iq({to:options.rooomName,type:"set"}).c("query",{xmlns:Strophe.NS.MUC_OWNER}).c("x",{xmlns:"jabber:x:data",type:"submit"}),this.context.stropheConn.sendIQ(roomiq.tree(),suc,err)},connection.prototype.join=function(options){var roomJid=this.context.appKey+"_"+options.roomId+"@conference."+this.domain,room_nick=roomJid+"/"+this.context.userId,suc=options.success||EMPTYFN,err=options.error||EMPTYFN,errorFn=function(ele){err({type:EASEMOB_IM_CONNCTION_JOINROOM_ERROR,msg:"加入房间失败",data:ele})},iq=$pres({from:this.context.jid,to:room_nick}).c("x",{xmlns:Strophe.NS.MUC});this.context.stropheConn.sendIQ(iq.tree(),suc,errorFn)},connection.prototype.listRooms=function(options){var iq;iq=$iq({to:options.server||"conference."+this.domain,from:this.context.jid,type:"get"}).c("query",{xmlns:Strophe.NS.DISCO_ITEMS});var suc=options.success||EMPTYFN,completeFn=function(result){var rooms=[];rooms=_parseRoomFn(result),suc(rooms)},err=options.error||EMPTYFN,errorFn=function(ele){err({type:EASEMOB_IM_CONNCTION_GETROOM_ERROR,msg:"获取群组列表失败",data:ele})};this.context.stropheConn.sendIQ(iq.tree(),completeFn,errorFn)},connection.prototype.queryRoomMember=function(options){var members=(this.domain,[]),iq=$iq({to:this.context.appKey+"_"+options.roomId+"@conference."+this.domain,type:"get"}).c("query",{xmlns:Strophe.NS.MUC+"#admin"}).c("item",{affiliation:"member"}),suc=options.success||EMPTYFN,completeFn=function(result){var items=result.getElementsByTagName("item");if(items)for(var i=0;i<items.length;i++){var item=items[i],mem={jid:item.getAttribute("jid"),affiliation:"member"};members.push(mem)}suc(members)},err=options.error||EMPTYFN,errorFn=function(ele){err({type:EASEMOB_IM_CONNCTION_GETROOMMEMBER_ERROR,msg:"获取群组成员列表失败",data:ele})};this.context.stropheConn.sendIQ(iq.tree(),completeFn,errorFn)},connection.prototype.queryRoomInfo=function(options){var domain=this.domain,iq=$iq({to:this.context.appKey+"_"+options.roomId+"@conference."+domain,type:"get"}).c("query",{xmlns:Strophe.NS.DISCO_INFO}),suc=options.success||EMPTYFN,members=[],completeFn=function(result){var fields=result.getElementsByTagName("field");if(fields)for(var i=0;i<fields.length;i++){var field=fields[i];if("owner"==field.getAttribute("label")){var mem={jid:(field.textContent||field.text)+"@"+domain,affiliation:"owner"};members.push(mem)}}suc(members)},err=options.error||EMPTYFN,errorFn=function(ele){err({type:EASEMOB_IM_CONNCTION_GETROOMINFO_ERROR,msg:"获取群组信息失败",data:ele})};this.context.stropheConn.sendIQ(iq.tree(),completeFn,errorFn)},connection.prototype.queryRoomOccupants=function(options){var suc=options.success||EMPTYFN,completeFn=function(result){var occupants=[];occupants=_parseRoomOccupantsFn(result),suc(occupants)},err=options.error||EMPTYFN,errorFn=function(ele){err({type:EASEMOB_IM_CONNCTION_GETROOMOCCUPANTS_ERROR,msg:"获取群组出席者列表失败",data:ele})},attrs={xmlns:Strophe.NS.DISCO_ITEMS},info=$iq({from:this.context.jid,to:this.context.appKey+"_"+options.roomId+"@conference."+this.domain,type:"get"}).c("query",attrs);this.context.stropheConn.sendIQ(info.tree(),completeFn,errorFn)},connection.prototype.setUserSig=function(desc){var dom=$pres({xmlns:"jabber:client"});desc=desc||"",dom.c("status").t(desc),this.sendCommand(dom.tree())},connection.prototype.setPresence=function(type,status){var dom=$pres({xmlns:"jabber:client"});type&&(status?(dom.c("show").t(type),dom.up().c("status").t(status)):dom.c("show").t(type)),this.sendCommand(dom.tree())},connection.prototype.getPresence=function(){var dom=$pres({xmlns:"jabber:client"});this.sendCommand(dom.tree())},connection.prototype.ping=function(options){options=options||{};var jid=_getJid(options,this),dom=$iq({from:this.context.jid||"",to:jid,type:"get"}).c("ping",{xmlns:"urn:xmpp:ping"});suc=options.success||EMPTYFN,error=options.error||this.onError;var failFn=function(ele){error({type:EASEMOB_IM_CONNCTION_PING_ERROR,msg:"ping失败",data:ele})};this.isOpened()?this.context.stropheConn.sendIQ(dom.tree(),suc,failFn):error({type:EASEMOB_IM_CONNCTION_OPEN_ERROR,msg:"连接还未建立,请先登录或等待登录处理完毕"})},connection.prototype.isOpened=function(){var status=this.context.status;return status==STATUS_OPENED},connection.prototype.isOpening=function(){var status=this.context.status;return status==STATUS_DOLOGIN_USERGRID||status==STATUS_DOLOGIN_IM},connection.prototype.isClosing=function(){var status=this.context.status;return status==STATUS_CLOSING},connection.prototype.isClosed=function(){var status=this.context.status;return status==STATUS_CLOSED},connection.prototype.clear=function(){this.context={status:STATUS_INIT}},connection}(),EMPTYFN=function(){};tempIndex=0,EASEMOB_IM_CONNCTION_USER_NOT_ASSIGN_ERROR=tempIndex++,EASEMOB_IM_CONNCTION_OPEN_ERROR=tempIndex++,EASEMOB_IM_CONNCTION_AUTH_ERROR=tempIndex++,EASEMOB_IM_CONNCTION_OPEN_USERGRID_ERROR=tempIndex++,EASEMOB_IM_CONNCTION_ATTACH_ERROR=tempIndex++,EASEMOB_IM_CONNCTION_ATTACH_USERGRID_ERROR=tempIndex++,EASEMOB_IM_CONNCTION_REOPEN_ERROR=tempIndex++,EASEMOB_IM_CONNCTION_SERVER_CLOSE_ERROR=tempIndex++,EASEMOB_IM_CONNCTION_SERVER_ERROR=tempIndex++,EASEMOB_IM_CONNCTION_IQ_ERROR=tempIndex++,EASEMOB_IM_CONNCTION_PING_ERROR=tempIndex++,EASEMOB_IM_CONNCTION_NOTIFYVERSION_ERROR=tempIndex++,EASEMOB_IM_CONNCTION_GETROSTER_ERROR=tempIndex++,EASEMOB_IM_CONNCTION_CROSSDOMAIN_ERROR=tempIndex++,EASEMOB_IM_CONNCTION_LISTENING_OUTOF_MAXRETRIES=tempIndex++,EASEMOB_IM_CONNCTION_RECEIVEMSG_CONTENTERROR=tempIndex++,EASEMOB_IM_CONNCTION_JOINROOM_ERROR=tempIndex++,EASEMOB_IM_CONNCTION_GETROOM_ERROR=tempIndex++,EASEMOB_IM_CONNCTION_GETROOMINFO_ERROR=tempIndex++,EASEMOB_IM_CONNCTION_GETROOMMEMBER_ERROR=tempIndex++,EASEMOB_IM_CONNCTION_GETROOMOCCUPANTS_ERROR=tempIndex++,EASEMOB_IM_UPLOADFILE_BROWSER_ERROR=tempIndex++,EASEMOB_IM_UPLOADFILE_ERROR=tempIndex++,EASEMOB_IM_UPLOADFILE_NO_LOGIN=tempIndex++,EASEMOB_IM_UPLOADFILE_NO_FILE=tempIndex++,EASEMOB_IM_DOWNLOADFILE_ERROR=tempIndex++,EASEMOB_IM_DOWNLOADFILE_NO_LOGIN=tempIndex++,EASEMOB_IM_DOWNLOADFILE_BROWSER_ERROR=tempIndex++,EASEMOB_IM_RESISTERUSER_ERROR=tempIndex++,tempIndex=0,EASEMOB_IM_MESSAGE_REC_TEXT=tempIndex++,EASEMOB_IM_MESSAGE_REC_TEXT_ERROR=tempIndex++,EASEMOB_IM_MESSAGE_REC_EMOTION=tempIndex++,EASEMOB_IM_MESSAGE_REC_PHOTO=tempIndex++,EASEMOB_IM_MESSAGE_REC_AUDIO=tempIndex++,EASEMOB_IM_MESSAGE_REC_AUDIO_FILE=tempIndex++,EASEMOB_IM_MESSAGE_REC_VEDIO=tempIndex++,EASEMOB_IM_MESSAGE_REC_VEDIO_FILE=tempIndex++,EASEMOB_IM_MESSAGE_REC_FILE=tempIndex++,EASEMOB_IM_MESSAGE_SED_TEXT=tempIndex++,EASEMOB_IM_MESSAGE_SED_EMOTION=tempIndex++,EASEMOB_IM_MESSAGE_SED_PHOTO=tempIndex++,EASEMOB_IM_MESSAGE_SED_AUDIO=tempIndex++,EASEMOB_IM_MESSAGE_SED_AUDIO_FILE=tempIndex++,EASEMOB_IM_MESSAGE_SED_VEDIO=tempIndex++,EASEMOB_IM_MESSAGE_SED_VEDIO_FILE=tempIndex++,EASEMOB_IM_MESSAGE_SED_FILE=tempIndex++,EASEMOB_IM_FILESIZE_LIMIT=10485760,tempIndex=0;var STATUS_INIT=tempIndex++,STATUS_DOLOGIN_USERGRID=tempIndex++,STATUS_DOLOGIN_IM=tempIndex++,STATUS_OPENED=tempIndex++,STATUS_CLOSING=tempIndex++,STATUS_CLOSED=tempIndex++;delete tempIndex,Easemob.im.Connection=Connection,Easemob.im.Utils=Utils,window.Easemob=Easemob}(window,void 0),function(window,undefined){"use strict";var EasemobWidget=EasemobWidget||{};EasemobWidget.utils=EasemobWidget.utils||{},EasemobWidget.utils.queryString=function(url,key){var r=url.match(new RegExp("[?&]?"+key+"=[0-9a-zA-Z%@._-]*[^&]","g"));return r=r&&r[0]?"?"==r[0][0]||"&"==r[0][0]?r[0].slice(1):r[0]:"",r.slice(key.length+1)},EasemobWidget.utils.getConfig=function(key){var that;that=key?key:location.href;var obj={};if(!that)return{str:"",json:obj,domain:""};var tmp,idx=that.indexOf("?"),sIdx=that.indexOf("//")>-1?that.indexOf("//"):0,domain=that.slice(sIdx,that.indexOf("/",sIdx+2)),arr=that.slice(idx+1).split("&");obj.src=that.slice(0,idx);for(var i=0,l=arr.length;l>i;i++)tmp=arr[i].split("="),obj[tmp[0]]=tmp.length>1?tmp[1]:"";return{str:that,json:obj,domain:domain}},EasemobWidget.utils.isAndroid=/Android/i.test(navigator.userAgent),EasemobWidget.utils.isMobile=/mobile/i.test(navigator.userAgent);var _on=function(target,ev,fn){target.addEventListener?target.addEventListener(ev,fn):target.attachEvent?target.attachEvent("on"+ev,fn):target["on"+ev]=fn},_remove=function(target,ev,fn){target.removeEventListener?target.removeEventListener(ev,fn):target.detachEvent?target.detachEvent("on"+ev,fn):target["on"+ev]=null};EasemobWidget.utils.on=_on,EasemobWidget.utils.remove=_remove,EasemobWidget.utils.isMin=function(){return document.visibilityState&&"hidden"==document.visibilityState||document.hidden?!0:void 0};var EmMessage=function(){var _supportPostMessage="postMessage"in window,_hasHash=function(url){var idx=url.lastIndexOf("/"),idxj=url.lastIndexOf("#");return 0>idxj?!1:url.indexOf("#")>idx&&idxj!=url.length?!0:void 0},_parseHash=function(url,key){var res=url.match(new RegExp(key+"\\w*"+key,"g"));return res&&res[0]?res[0].slice(key.length,-key.length):""},_getMsg=function(key,url){var str=key.toString(),arr=url.match(new RegExp(str+"\\w*"+str,"g"));return arr?arr[0].slice(str.length,-str.length):""},_appendMsg=function(key,msg,url){return url.replace(new RegExp(key+"\\w*"+key,"g"),key+msg+key)},message=function(iframeId,prefix){return this instanceof message?(this.t=(new Date).getTime(),this.iframe=document.getElementById(iframeId),this.prefix=prefix||"_em_",void delete this.t):new message};return message.prototype.sendToParent=function(msg){if("string"!=typeof msg)throw"msg must be string";return _supportPostMessage?(window.parent.postMessage(msg,"*"),this):this},message.prototype.sendToIframe=function(msg){if("string"!=typeof msg)throw"msg must be string";if(_supportPostMessage)return this.iframe.contentWindow.postMessage(msg,"*"),this;var src=this.iframe.getAttribute("src");return _hasHash(src)?_getMsg(this.prefix,src)?this.iframe.setAttribute("src",_appendMsg(this.prefix,msg,src)):this.iframe.setAttribute("src",src+"&"+this.prefix+msg+this.prefix):this.iframe.setAttribute("src",src+="#"+this.prefix+msg+this.prefix),this},message.prototype.listenToParent=function(callback){if(_supportPostMessage)return _on(window,"message",function(e){callback(e.data)}),this;if(window.onhashchange)window.onhashchange=function(){callback(_parseHash(location.href,this.prefix))};else{var that=this,memHref=location.href;setInterval(function(){var curHref=location.href;curHref!=memHref&&(memHref=curHref,callback(_parseHash(curHref,that.prefix)))},50)}return this},message.prototype.listenToIframe=function(callback){return _supportPostMessage&&_on(window,"message",function(e){callback(e.data)}),this},message}(),c={setcookie:function(key,value){var date=new Date;date.setTime(date.getTime()+2592e6),document.cookie=key+"="+escape(value)+";expires="+date.toGMTString()},getcookie:function(key){var results=document.cookie.match("(^|;) ?"+key+"=([^;]*)(;|$)");return results?unescape(results[2]):""}};Date.prototype.format=function(fmt){var o={"M+":this.getMonth()+1,"d+":this.getDate(),"h+":this.getHours(),"m+":this.getMinutes(),"s+":this.getSeconds()};/(y+)/.test(fmt)&&(fmt=fmt.replace(RegExp.$1,(this.getFullYear()+"").substr(4-RegExp.$1.length)));for(var k in o)new RegExp("("+k+")").test(fmt)&&(fmt=fmt.replace(RegExp.$1,1==RegExp.$1.length?o[k]:("00"+o[k]).substr((""+o[k]).length)));return fmt},window.EasemobWidget=EasemobWidget,window.EmMessage=EmMessage,window.Emc=c}(window,void 0),function($){$.fn.autogrow=function(options){return this.filter("textarea").each(function(){var $this=$(this),minHeight=$this.height(),shadow=($this.css("lineHeight"),$("<div>").css({position:"absolute",top:-1e4,left:-1e4,width:$(this).width(),fontSize:$this.css("fontSize"),fontFamily:$this.css("fontFamily"),lineHeight:$this.css("lineHeight"),resize:"none","word-wrap":"break-word"}).appendTo(document.body)),update=function(){var times=function(string,number){for(var i=0,r="";number>i;i++)r+=string;return r},val=this.value.replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/&/g,"&amp;").replace(/\n$/,"<br/>&nbsp;").replace(/\n/g,"<br/>").replace(/ {2,}/g,function(space){return times("&nbsp;",space.length-1)+" "});shadow.html(val),val&&$(this).css("height",Math.max(shadow.height()+17,minHeight)),"function"==typeof options.callback&&options.callback()};$(this).change(update).keyup(update).keydown(update);var that=this;options.update=function(){update.apply(that)},update.apply(this)}),this}}(jQuery),EasemobWidget.LISTSPAN=20,EasemobWidget.PICTYPE={jpg:!0,gif:!0,png:!0,bmp:!0},EasemobWidget.LOADING='<div class="easemobWidget-loading"><svg version="1.1" id="图层_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"    viewBox="0 0 70 70" enable-background="new 0 0 70 70" xml:space="preserve">    <circle opacity="0.3" fill="none" stroke="#000000" stroke-width="4" stroke-miterlimit="10" cx="35" cy="35" r="11"/>    <path fill="none" stroke="#E5E5E5" stroke-width="4" stroke-linecap="round" stroke-miterlimit="10" d="M24,35c0-6.1,4.9-11,11-11    c2.8,0,5.3,1,7.3,2.8"/><image src="/webim/resources/loading.gif" width="20" style="margin-top:10px;" /></svg></div>',EasemobWidget.THEME={"天空之城":{css:"body .theme-color{color:#42b8f4;}body .bg-color{background-color:#42b8f4}.border-color{border:1px solid #00a0e7}.hover-color{background-color:#7dcdf7}"},"丛林物语":{css:"body .theme-color{color:#00b45f;}body .bg-color{background-color:#00b45f}.border-color{border:1px solid #009a51}.hover-color{background-color:#16cd77}"},"红瓦洋房":{css:"body .theme-color{color:#b50e03;}body .bg-color{background-color:#b50e03}.border-color{border:1px solid #811916}.hover-color{background-color:#e92b25}"},"鲜美橙汁":{css:"body .theme-color{color:#ffa000;}body .bg-color{background-color:#ffa000}.border-color{border:1px solid #f69000}.hover-color{background-color:#ffb63b}"},"青草田间":{css:"body .theme-color{color:#9ec100;}body .bg-color{background-color:#9ec100}.border-color{border:1px solid #809a00}.hover-color{background-color:#bad921}"},"湖光山色":{css:"body .theme-color{color:#00cccd;}body .bg-color{background-color:#00cccd}.border-color{border:1px solid #12b3b4}.hover-color{background-color:#38e6e7}"},"冷峻山峰":{css:"body .theme-color{color:#5b799a;}body .bg-color{background-color:#5b799a}.border-color{border:1px solid #48627b}.hover-color{background-color:#6a8eb5}"},"月色池塘":{css:"body .theme-color{color:#3977cf;}body .bg-color{background-color:#3977cf}.border-color{border:1px solid #2b599b}.hover-color{background-color:#548bdc}"}},Easemob.im.EMOTIONS={path:"resources/faces/",map:{"[):]":"ee_1.png","[:D]":"ee_2.png","[;)]":"ee_3.png","[:-o]":"ee_4.png","[:p]":"ee_5.png","[(H)]":"ee_6.png","[:@]":"ee_7.png","[:s]":"ee_8.png","[:$]":"ee_9.png","[:(]":"ee_10.png","[:'(]":"ee_11.png","[:|]":"ee_12.png","[(a)]":"ee_13.png","[8o|]":"ee_14.png","[8-|]":"ee_15.png","[+o(]":"ee_16.png","[<o)]":"ee_17.png","[|-)]":"ee_18.png","[*-)]":"ee_19.png","[:-#]":"ee_20.png","[:-*]":"ee_21.png","[^o)]":"ee_22.png","[8-)]":"ee_23.png","[(|)]":"ee_24.png","[(u)]":"ee_25.png","[(S)]":"ee_26.png","[(*)]":"ee_27.png","[(#)]":"ee_28.png","[(R)]":"ee_29.png","[({)]":"ee_30.png","[(})]":"ee_31.png","[(k)]":"ee_32.png","[(F)]":"ee_33.png","[(W)]":"ee_34.png","[(D)]":"ee_35.png"}},EasemobWidget.api={getTo:function(tenantId){var getTo=$.Deferred(function(){$.ajax({url:"/v1/webimplugin/targetChannels",data:{tenantId:tenantId},cache:!1}).done(function(info){getTo.resolve(info)}).fail(function(){getTo.reject()})});return getTo},getStatus:function(tenantId){var getStatus=$.Deferred(function(){$.ajax({url:"/v1/webimplugin/timeOffDuty",data:{tenantId:tenantId},cache:!1}).done(function(info){getStatus.resolve(info)}).fail(function(){getStatus.reject()})});return getStatus},getTheme:function(tenantId){var getTheme=$.Deferred(function(){$.ajax({url:"/v1/webimplugin/theme/options",data:{tenantId:tenantId},cache:!1}).done(function(info){
getTheme.resolve(info)}).fail(function(){getTheme.reject()})});return getTheme},getWord:function(tenantId){var getWord=$.Deferred(function(){$.ajax({url:"/v1/webimplugin/notice/options",data:{tenantId:tenantId},cache:!1}).done(function(info){getWord.resolve(info)}).fail(function(){getWord.reject()})});return getWord},getPwd:function(obj){var getPwd=$.Deferred(function(){$.ajax({url:"/v1/webimplugin/visitors/password",data:{userId:obj.user},cache:!1}).done(function(info){getPwd.resolve(info)}).fail(function(){getPwd.reject()})});return getPwd},getGroup:function(obj){var getGroup=$.Deferred(function(){$.ajax({url:"/v1/webimplugin/visitors/"+obj.user+"/ChatGroupId?techChannelInfo="+escape(obj.orgName+"#"+obj.appName+"#"+obj.to),cache:!1}).done(function(info){getGroup.resolve(info)}).fail(function(){getGroup.reject()})});return getGroup},getHistory:function(from,size,chatGroupId,tenantId){var getHistory=$.Deferred(function(){$.ajax({url:"/v1/webimplugin/visitors/msgHistory",data:{fromSeqId:from,size:size,chatGroupId:chatGroupId,tenantId:tenantId},cache:!1}).done(function(info){getHistory.resolve(info)}).fail(function(){getHistory.reject()})});return getHistory},getUser:function(obj){var getUser=$.Deferred(function(){$.ajax({url:"/v1/webimplugin/visitors",contentType:"application/json",type:"post",data:JSON.stringify({orgName:obj.orgName,appName:obj.appName,imServiceNumber:obj.to})}).done(function(info){getUser.resolve(info)}).fail(function(){getUser.reject()})});return getUser}},EasemobWidget.init=function(obj,callback){window.URL=window.URL||window.webkitURL||window.mozURL||window.msURL,"undefined"!=typeof HTMLAudioElement&&(HTMLAudioElement.prototype.stop=function(){this.pause(),this.currentTime=0});var wrapper=$("#normal"),message=new EmMessage,tenantId=obj.json.tenantId;$.when(EasemobWidget.api.getTo(tenantId),EasemobWidget.api.getStatus(tenantId),EasemobWidget.api.getTheme(tenantId),EasemobWidget.api.getWord(tenantId)).done(function(toinfo,sinfo,tinfo,winfo){if(obj.offline=sinfo,toinfo.length>0){obj.to=toinfo[0].imServiceNumber,obj.orgName=toinfo[0].orgName,obj.appName=toinfo[0].appName,obj.tenantName=toinfo[0].tenantName,obj.appkey=toinfo[0].orgName+"#"+toinfo[0].appName,obj.theme=tinfo&&tinfo.length?tinfo[0].optionValue:"天空之城",obj.word=winfo&&winfo.length?winfo[0].optionValue:"";var curUser;obj.root?(curUser=Emc.getcookie("emKefuChannel")!=obj.to+"*"+obj.orgName+"*"+obj.appName?null:Emc.getcookie("emKefuUser"),Emc.setcookie("emKefuChannel",obj.to+"*"+obj.orgName+"*"+obj.appName)):(curUser=obj.json.c!=obj.to+"*"+obj.orgName+"*"+obj.appName?null:obj.json.u,message.sendToParent("setchannel@"+obj.to+"*"+obj.orgName+"*"+obj.appName)),curUser?(obj.user=curUser,$.when(EasemobWidget.api.getPwd(obj),EasemobWidget.api.getGroup(obj)).done(function(p,g){wrapper.attr("data-group",g),obj.password=p,"function"==typeof callback&&callback()}).fail(function(){})):(wrapper.attr("data-history",1),$.when(EasemobWidget.api.getUser(obj)).done(function(info){obj.user=info.userId,obj.password=info.userPassword,obj.root?Emc.setcookie("emKefuUser",obj.user):message.sendToParent("setuser@"+obj.user),"function"==typeof callback&&callback()}))}}).fail(function(){})},function(window,undefined){"use strict";var main=function(config){var sendQueue={},userHash={},isGroupChat=!1,isGroupOpened=!1,isShowDirect=!1,curGroup="",swfupload=null,https="https:"==location.protocol?!0:!1,click=EasemobWidget.utils.isMobile&&"ontouchstart"in window?"touchstart":"click";config.root=window.top==window,config.json.hide="false"==config.json.hide?!1:config.json.hide,EasemobWidget.init(config,function(){userHash.normal={user:config.user,password:config.password},im.init()});var im={init:function(){if(this.getDom(),this.changeTheme(),config.root&&(this.min.addClass("hide"),this.toggleChatWindow()),this.handleFixedBtn(),this.fillFace(),this.setWord(),this.setTitle(config.json.emgroup?unescape(config.json.emgroup):""),this.audioAlert(),this.mobileInit(),this.setOffline(),this.bindEvents(),this.open(),this.getHistory(0,$("#normal"),function(wrapper,info){config.history=info,im.handleHistory(wrapper)}),config.json.emgroup&&config.root){var value=unescape(config.json.emgroup);this.handleGroup(value),userHash[value]=userHash[value]||{},userHash[value].user=Emc.getcookie(escape(value)),curGroup=value,handleGroupUser(curGroup)}},paste:function(obj,callback){$(obj).on("paste",function(e){var ev=e.originalEvent;try{if(ev.clipboardData&&ev.clipboardData.types)ev.clipboardData.items.length>0&&/^image\/\w+$/.test(ev.clipboardData.items[0].type)&&callback instanceof Function&&callback(ev.clipboardData.items[0].getAsFile());else if(window.clipboardData){var url=window.clipboardData.getData("URL");callback instanceof Function&&callback(url)}}catch(e){callback instanceof Function&&callback()}})},getConnection:function(){return new Easemob.im.Connection({https:https?!0:!1,url:(https?"https:":"http:")+"//im-api.easemob.com/http-bind/"})},getHistory:function(from,wrapper,callback){wrapper=wrapper||im.chatWrapper,wrapper.data("group")&&$.when(EasemobWidget.api.getHistory(from,EasemobWidget.LISTSPAN,wrapper.data("group"),config.json.tenantId)).done(function(info){info&&info.length==EasemobWidget.LISTSPAN?(wrapper.attr("data-start",Number(info[EasemobWidget.LISTSPAN-1].chatGroupSeqId)-1),wrapper.attr("data-history",0)):wrapper.attr("data-history",1),callback instanceof Function&&callback(wrapper,info)})},setAttribute:function(){return this.msgCount=0,this.scbT=0,this.autoGrowOptions={},this.historyFirst=!0,this.msgTimeSpan={},this},handleGroup:function(type){if("string"==typeof type)type=unescape(type),im.group=type,im.handleChatContainer(im.group);else{if(!im.group)return void(type.ext&&type.ext.weichat&&type.ext.weichat.queueName&&delete type.ext.weichat.queueName);type.ext=type.ext||{},type.ext.weichat=type.ext.weichat||{},type.ext.weichat.queueName=im.group}},handleChatContainer:function(groupId){var curChatContainer=$(document.getElementById(groupId));curChatContainer.length>0?(this.chatWrapper=curChatContainer,this.setTitle(groupId),curChatContainer.removeClass("hide").siblings(".easemobWidget-chat").addClass("hide")):(curChatContainer=$('<div data-start="0" data-history="1" id="'+groupId+'" class="easemobWidget-chat"></div>'),this.chatWrapper.parent().prepend(curChatContainer),this.handleChatContainer(groupId))},handleHistory:function(cwrapper){config.history&&config.history.length>0&&($.each(config.history,function(k,v){var wrapper=cwrapper||this.chatWrapper;im.addDate(v.timestamp||v.body.timestamp,!0,wrapper);var msg=v.body;if(v.body&&v.body.bodies.length>0){var msg=v.body.bodies[0];if(v.body.from&&v.body.from.indexOf("webim-visitor")>-1){if(v.body.ext&&v.body.ext.weichat&&v.body.ext.weichat.ctrlType&&"enquiry"==v.body.ext.weichat.ctrlType)return;switch(msg.type){case"img":im.sendImgMsg(msg,wrapper);break;case"txt":im.sendTextMsg(msg,wrapper,!0)}}else v.body.ext&&v.body.ext.weichat&&v.body.ext.weichat.ctrlType&&"inviteEnquiry"==v.body.ext.weichat.ctrlType&&(msg=v.body),v.body.ext&&v.body.ext.msgtype&&v.body.ext.msgtype&&(msg=v.body),im.receiveMsg(msg,msg.type,"history",wrapper)}}),im.historyFirst&&(im.chatWrapper.find("img:last").on("load",im.scrollBottom),im.scrollBottom(),im.historyFirst=!1))},setTitle:function(title){var nickName=this.headBar.find(".easemobWidgetHeader-nickname");nickName.html(config.tenantName+(title?"-"+title:"")),document.title=nickName.html()+(title?"":"-客服")},mobileInit:function(){EasemobWidget.utils.isMobile&&(this.Im.find(".easemobWidget-logo").hide(),config.json.hide||config.root||(this.fixedBtn.css({width:"100%",top:"0"}),this.fixedBtn.children().css({width:"100%","border-radius":"0","text-align":"center","font-size":"18px",height:"40px","line-height":"40px"})),this.word.find("span").css("font-size","14px"),this.Im.find(".js_drag").css("text-align","center"),this.evaluate.addClass("hide"),this.mobileLink.attr("href",location.href),this.sendbtn.removeClass("disabled").addClass("easemobWidgetSendBtn-mobile"),this.satisDialog.addClass("easemobWidget-satisfaction-dialog-mobile"),this.headBar.addClass("easemobWidgetHeader-mobile"),this.chatWrapper.parent().addClass("easemobWidgetBody-mobile"),this.faceWrapper.parent().addClass("easemobWidget-face-wrapper-mobile"),this.facebtn.addClass("easemobWidget-face-mobile"),$(".easemobWidget-face-bg").addClass("easemobWidget-face-bg-mobile"),this.uploadbtn.addClass("easemobWidget-file-mobile"),this.sendbtn.parent().addClass("easemobWidgetSend-mobile"),this.textarea.addClass("textarea-mobile"),this.Im.find(".easeWidget-face-rec").addClass("easeWidget-face-rec-mobile"))},setWord:function(){config.word?this.word.find("span").html(Easemob.im.Utils.parseLink(config.word)):(this.word.addClass("hide"),this.chatWrapper.parent().css("top","43px"))},fillFace:function(){var faceStr='<li class="e-face">',count=0;$.each(Easemob.im.EMOTIONS.map,function(k,v){count+=1,faceStr+="<div class='easemobWidget-face-bg e-face'>                                    <img class='easemobWidget-face-img e-face'                                         src='"+Easemob.im.EMOTIONS.path+v+"'                                         data-value="+k+" />                                </div>",count%7==0&&(faceStr+='</li><li class="e-face">')}),count%7==0?faceStr=faceStr.slice(0,-'<li class="e-face">'.length):faceStr+="</li>",this.faceWrapper.html(faceStr),faceStr=null},errorPrompt:function(msg){var me=this;me.ePrompt.html(msg).removeClass("hide"),setTimeout(function(){me.ePrompt.html(msg).addClass("hide")},2e3)},changeTheme:function(){if(config.json.color){var color=config.json.color;this.min.css("background-color",color),this.fixedBtn.children().css("background-color",color),this.headBar.css("background-color",color),this.sendbtn.css("background-color",color)}else config.theme&&(EasemobWidget.THEME[config.theme]||(config.theme="天空之城"),$('<style type="text/css">'+EasemobWidget.THEME[config.theme].css+"</style>").appendTo("head"))},handleFixedBtn:function(){config.json.hide||config.root||this.fixedBtn.removeClass("hide")},setOffline:function(){var me=this;return config.offline?(me.fixedBtn.find("a").addClass("easemobWidget-offline-bg"),me.headBar.find(".easemobWidgetHeader-bar").removeClass("online").addClass("offline"),me.offline.removeClass("hide"),me.word.addClass("hide"),me.chatWrapper.parent().addClass("hide"),me.sendbtn.parent().addClass("hide"),void me.dutyStatus.html("(离线)")):(me.offline.addClass("hide"),config.word&&me.word.removeClass("hide"),me.chatWrapper.parent().removeClass("hide"),me.sendbtn.parent().removeClass("hide"),me.dutyStatus.html("(在线)"),me.headBar.find(".easemobWidgetHeader-bar").removeClass("offline").addClass("online"),void me.fixedBtn.find("a").removeClass("easemobWidget-offline-bg"))},toggleChatWindow:function(windowStatus){var me=this;config.root?me.Im.removeClass("hide"):(setTimeout(function(){!config.json.hide&&me.fixedBtn.toggleClass("hide")},100),message.sendToParent("show"==windowStatus||me.Im.hasClass("hide")?"showChat":"minChat"),"show"==windowStatus?(me.fixedBtn.removeClass("hide"),me.Im.removeClass("hide")):me.Im.toggleClass("hide")),me.Im.hasClass("hide")?me.isOpened=!1:(me.textarea.focus(),me.isOpened=!0,me.scrollBottom()),me.addPrompt()},sdkInit:function(conn){var me=this;conn.listen({onOpened:function(){for(conn.setPresence(),conn.heartBeat(conn),isGroupChat&&(isGroupOpened=!0);sendQueue[curGroup]&&sendQueue[curGroup].length;)conn.send(sendQueue[curGroup].pop())},onTextMessage:function(message){me.receiveMsg(message,"txt")},onEmotionMessage:function(message){me.receiveMsg(message,"face")},onPictureMessage:function(message){me.receiveMsg(message,"img")},onLocationMessage:function(message){me.receiveMsg(message,"location")},onAudioMessage:function(message){me.receiveMsg(message,"audio")},onClosed:function(){me.open()},onError:function(e){for(conn.stopHeartBeat(conn);sendQueue[curGroup]&&sendQueue[curGroup].length;)conn.send(sendQueue[curGroup].pop());switch(e.type){case 1:me.open();break;case 3:case 6:conn.close();break;case 7:conn.isOpened()?conn.close():(conn.isClosed()||conn.isClosing())&&me.open();break;case 8:}}})},addDate:function(date,isHistory,wrapper){var htmlPre='<div class="easemobWidget-date">',htmlEnd="</div>",fmt="M月d日 hh:mm";wrapper=wrapper||this.chatWrapper;var id=wrapper.attr("id");date?$(htmlPre+new Date(date).format(fmt)+htmlEnd).insertAfter(wrapper.find("div:first")):isHistory||((!this.msgTimeSpan[id]||(new Date).getTime()-this.msgTimeSpan[id]>6e4)&&wrapper.append(htmlPre+(new Date).format(fmt)+htmlEnd),this.resetSpan(id))},resetSpan:function(id){this.msgTimeSpan[id]=(new Date).getTime()},open:function(){var me=this,key=isGroupChat?curGroup:"normal";userHash[key].conn||(userHash[key].conn=me.getConnection(),me.sdkInit(userHash[key].conn)),me.conn=userHash[key].conn,me.conn.isOpened()||me.conn.open({user:userHash[key].user,pwd:userHash[key].password,appKey:config.appkey})},getDom:function(){this.offline=$("#easemobWidgetOffline"),this.leaveMsgBtn=this.offline.find("button"),this.contact=this.offline.find("input"),this.leaveMsg=this.offline.find("textarea"),this.fixedBtn=$("#easemobWidgetPopBar"),this.Im=$(".easemobWidgetWrapper"),this.pasteWrapper=$(".easemobWidget-paste-wrapper"),this.audio=$("audio").get(0),this.chatWrapper=this.Im.find(".easemobWidget-chat"),this.textarea=this.Im.find(".easemobWidget-textarea"),this.sendbtn=this.Im.find("#easemobWidgetSendBtn"),this.evaluate=this.sendbtn.parent().find(".easemobWidget-satisfaction"),this.facebtn=this.Im.find(".easemobWidget-face"),this.uploadbtn=this.Im.find("#easemobWidgetFile"),this.realfile=this.Im.find("#easemobWidgetFileInput"),this.faceWrapper=this.Im.find(".easemobWidget-face-container"),this.headBar=this.Im.find("#easemobWidgetHeader"),this.min=this.Im.find(".easemobWidgetHeader-min"),this.closeWord=this.Im.find(".easemobWidget-word-close"),this.word=this.Im.find(".easemobWidget-word"),this.messageCount=this.fixedBtn.find(".easemobWidget-msgcount"),this.ePrompt=this.Im.find(".easemobWidget-error-prompt"),this.mobileLink=this.Im.find("#easemobWidgetLink"),this.dutyStatus=this.Im.find(".easemobWidgetHeader-word-status"),this.satisDialog=this.Im.find(".easemobWidget-satisfaction-dialog")},audioAlert:function(){return},face:function(msg){return $.isArray(msg)?msg="["+msg[0]+"]":/\[.*\]/.test(msg)&&(msg=msg.replace(/&amp;/g,"&"),msg=msg.replace(/&#39;/g,"'"),msg=msg.replace(/&lt;/g,"<"),$.each(Easemob.im.EMOTIONS.map,function(k,v){for(;msg.indexOf(k)>=0;)msg=msg.replace(k,'<img class="chat-face-all" src="'+Easemob.im.EMOTIONS.path+Easemob.im.EMOTIONS.map[k]+'">')})),msg},toggleFaceWrapper:function(e){var h=im.sendbtn.parent().outerHeight();return im.faceWrapper.parent().css("bottom",h+"px").toggleClass("hide"),!1},bindEvents:function(){var me=this;me.Im.on("mouseenter",".easemobWidget-list-btn button",function(){$(this).addClass("bg-color")}).on("mouseleave",".easemobWidget-list-btn button",function(){$(this).removeClass("bg-color")}).on("touchstart",".easemobWidget-list-btn button",function(){$(this).addClass("bg-color")}).on("touchend",".easemobWidget-list-btn button",function(){$(this).removeClass("bg-color")}),function(){var f=null;me.paste(me.textarea,function(blob){blob&&(me.pasteWrapper.removeClass("hide"),window.URL.createObjectURL&&(f=blob)&&me.pasteWrapper.find(".easemobWidget-paste-image").append($('<img src="'+window.URL.createObjectURL(blob)+'"/>')))}),me.pasteWrapper.on("click",".js_cancelsend",function(){me.pasteWrapper.find(".easemobWidget-paste-image").html(""),me.pasteWrapper.addClass("hide")}),me.pasteWrapper.on("click",".js_sendimg",function(){return f?(me.sendImgMsg(null,null,{data:f,url:me.pasteWrapper.find(".easemobWidget-paste-image img").attr("src")},me.conn.getUniqueId()),me.pasteWrapper.find(".easemobWidget-paste-image").html(""),void me.pasteWrapper.addClass("hide")):!1})}(),"onpopstate"in window&&$(window).on("popstate",me.open),me.Im.on(click,".easemobWidget-msg-status",function(){var that=$(this),w=that.parent().parent(),id=w.attr("id");that.addClass("hide"),w.find(".easemobWidget-msg-loading").removeClass("hide"),me.send(id)}),me.headBar.find(".js_drag").on("mousedown",function(e){var ev=e.originalEvent;return me.textarea.blur(),message.sendToParent("dragready"+ev.clientX+"&"+ev.clientY),!1}).on("mouseup",function(){return message.sendToParent("dragend"),!1}),me.evaluate.on(click,function(){me.satisDialog.get(0).inviteId="",me.satisDialog.get(0).serviceSessionId="",me.satisDialog.removeClass("hide")}),me.Im.on(click,".js_satisfybtn button",function(){var that=$(this);return me.satisDialog.get(0).inviteId=that.data("inviteid"),me.satisDialog.get(0).serviceSessionId=that.data("servicesessionid"),me.satisDialog.removeClass("hide"),!1}),me.satisDialog.on(click,"i, .js_cancel",function(){me.satisDialog.addClass("hide")}),me.satisDialog.on(click,".js_satisfy",function(){var suc=me.satisDialog.find(".js_suc"),level=me.satisDialog.find("li.sel").length,text=me.satisDialog.find("textarea");return 0==level?(me.errorPrompt("请先选择星级"),!1):(me.sendSatisfaction(level,text.val()),text.blur(),suc.removeClass("hide"),void setTimeout(function(){text.val(""),$.each(me.satisDialog.find("li.sel"),function(k,v){$(v).removeClass("sel")}),suc.addClass("hide"),me.satisDialog.addClass("hide")},1500))}),me.satisDialog.on(click,"li",function(e){e.originalEvent.preventDefault&&e.originalEvent.preventDefault();for(var that=$(this),par=that.parent(),temp=par.find("li"),i=0;5>i;i++)i<=that.index()?temp.eq(i).addClass("sel"):temp.eq(i).removeClass("sel");e.originalEvent.stopPropagation&&e.originalEvent.stopPropagation()}),me.Im.on(click,".js_robertbtn button",function(){var that=$(this);return me.sendTextMsg(that.html()),!1}),me.closeWord.on(click,function(){me.word.fadeOut(),me.chatWrapper.parent().css("top","43px")}),me.autoGrowOptions.callback=function(){var h=im.sendbtn.parent().outerHeight();im.faceWrapper.parent().css("bottom",h+"px")},EasemobWidget.utils.isMobile&&me.textarea.autogrow(me.autoGrowOptions),me.textarea.on("keyup change",function(){$(this).val()?me.sendbtn.removeClass("disabled"):me.sendbtn.addClass("disabled")}).on("touchstart",function(){me.scrollBottom(800),me.textarea.css("overflow-y","auto")}).on("blur",function(){}),EasemobWidget.utils.isMobile&&me.textarea.on("input",function(){me.autoGrowOptions.update(),me.scrollBottom(800)}),me.min.on("mouseenter mouseleave",function(){$(this).toggleClass("hover-color")}).on("click",function(e){return me.toggleChatWindow(),!1}),me.facebtn.on(click,me.toggleFaceWrapper).on("mouseenter",function(){$(this).addClass("theme-color")}).on("mouseleave",function(){$(this).removeClass("theme-color")}),me.faceWrapper.on(click,".easemobWidget-face-bg",function(e){e.originalEvent.preventDefault&&e.originalEvent.preventDefault(),!EasemobWidget.utils.isMobile&&me.textarea.focus(),me.textarea.val(me.textarea.val()+$(this).find("img").data("value")),EasemobWidget.utils.isMobile&&(me.autoGrowOptions.update(),setTimeout(function(){me.textarea.get(0).scrollTop=1e4},100)),me.sendbtn.removeClass("disabled"),e.originalEvent.stopPropagation&&e.originalEvent.stopPropagation()}),me.fixedBtn.find("a").on("click",function(){EasemobWidget.utils.isMobile?$(this).attr({target:"_blank",href:location.href}):(me.chatWrapper.removeClass("hide").siblings().addClass("hide"),me.toggleChatWindow(),me.scrollBottom())}),me.realfile.on("change",function(){me.sendImgMsg()}).on("click",function(){return Easemob.im.Utils.isCanUploadFile()?void 0:(me.errorPrompt("当前浏览器不支持发送图片"),!1)}),$(document).on(click,function(ev){var e=window.event||ev,t=$(e.srcElement||e.target);t.hasClass("e-face")||me.faceWrapper.parent().addClass("hide")}),$(".e-face, .easemobWidgetBody-wrapper").on("touchstart",function(e){me.textarea.blur(),!me.textarea.val()&&me.textarea.css("overflow-y","hidden")}),me.uploadbtn.on("click",function(){return Easemob.im.Utils.isCanUploadFile()?void me.realfile.click():(me.errorPrompt("当前浏览器不支持发送图片"),!1)}).on("mouseenter",function(){$(this).addClass("theme-color")}).on("mouseleave",function(){$(this).removeClass("theme-color")}),me.textarea.on("keydown",function(evt){var that=$(this);if(EasemobWidget.utils.isMobile&&13==evt.keyCode||evt.ctrlKey&&13==evt.keyCode||evt.shiftKey&&13==evt.keyCode)return that.val($(this).val()+"\n"),!1;if(13==evt.keyCode){if(me.faceWrapper.parent().addClass("hide"),me.sendbtn.hasClass("disabled"))return!1;me.sendTextMsg(),setTimeout(function(){that.val("")},0)}}),me.sendbtn.on("click",function(){return me.sendbtn.hasClass("disabled")?!1:(me.faceWrapper.parent().addClass("hide"),me.sendTextMsg(),void me.textarea.css({height:"34px",overflowY:"hidden"}).focus())}),me.leaveMsgBtn.on(click,function(){if(me.contact.val()||me.leaveMsg.val())if(me.contact.val())if(me.leaveMsg.val())if(/^\d{5,11}$/g.test(me.contact.val())||/^[a-zA-Z0-9-_]+@([a-zA-Z0-9-]+[.])+[a-zA-Z]+$/g.test(me.contact.val())){var opt={to:config.to,msg:"手机号码/邮箱/QQ号："+me.contact.val()+"   留言："+me.leaveMsg.val(),type:"chat"};me.handleGroup(opt),me.send(opt);var succeed=me.leaveMsgBtn.parent().find(".easemobWidget-success-prompt");succeed.removeClass("hide"),setTimeout(function(){succeed.addClass("hide")},1500),me.contact.val(""),me.leaveMsg.val("")}else me.errorPrompt("请输入正确的手机号码/邮箱/QQ号");else me.errorPrompt("留言不能为空");else me.errorPrompt("联系方式不能为空");else me.errorPrompt("联系方式和留言不能为空")});var st,_startY,_y,triggerGetHistory=function(){1!=me.chatWrapper.attr("data-history")&&$.when(EasemobWidget.api.getHistory(me.chatWrapper.attr("data-start"),EasemobWidget.LISTSPAN,me.chatWrapper.data("group"),config.json.tenantId)).done(function(info){if(info&&info.length==EasemobWidget.LISTSPAN){var start=Number(info[EasemobWidget.LISTSPAN-1].chatGroupSeqId)-1;0==start&&setTimeout(function(){me.chatWrapper.attr("data-history",1)},0),me.chatWrapper.attr("data-start",start),me.chatWrapper.attr("data-history",0)}else setTimeout(function(){me.chatWrapper.attr("data-history",1)},0);config.history=info,im.handleHistory()})};me.chatWrapper.parent().on("touchstart",function(e){var touch=e.originalEvent.touches;e.originalEvent.touches&&e.originalEvent.touches.length>0&&(_startY=touch[0].pageY)}).on("touchmove",function(e){var $t=$(this),touch=e.originalEvent.touches;e.originalEvent.touches&&e.originalEvent.touches.length>0&&(touch=e.originalEvent.touches,_y=touch[0].pageY,_y-_startY>8&&$t.scrollTop()<=50&&(clearTimeout(st),st=setTimeout(function(){triggerGetHistory()},100)))}),me.Im.on("mousewheel DOMMouseScroll",".easemobWidget-chat",function(e){var $t=$(this);(e.originalEvent.wheelDelta/120>0||e.originalEvent.detail<0)&&(clearTimeout(st),st=setTimeout(function(){Math.abs($t.offset().top)<=100&&triggerGetHistory()},400))})},scrollBottom:function(type){var ocw=im.chatWrapper.parent().get(0);type?(clearTimeout(this.scbT),this.scbT=setTimeout(function(){ocw.scrollTop=ocw.scrollHeight-ocw.offsetHeight+1e4},type)):ocw.scrollTop=ocw.scrollHeight-ocw.offsetHeight+1e4},sendImgMsg:function(msg,wrapper,file,msgId){var me=this;if(wrapper=wrapper||me.chatWrapper,msg){var temp=$("                        <div class='easemobWidget-right'>                            <div class='easemobWidget-msg-wrapper'>                                <i class='easemobWidget-right-corner'>J</i>                                <div class='easemobWidget-msg-status hide'><span>发送失败</span><i></i></div>                                <div class='easemobWidget-msg-container'>                                    <a class='easemobWidget-noline' href='"+msg.url+"' target='_blank'><img src='"+msg.url+"'/></a>                                </div>                            </div>                        </div>                    ");return void wrapper.prepend(temp)}var msgid=msgId||me.conn.getUniqueId();if(Easemob.im.Utils.isCanUploadFileAsync()){if(me.realfile.val())file=Easemob.im.Utils.getFileUrl(me.realfile.attr("id"));else if(!file)return;var temp=$("                        <div id='"+msgid+"' class='easemobWidget-right'>                            <div class='easemobWidget-msg-wrapper'>                                <i class='easemobWidget-right-corner'>J</i>                                <div class='easemobWidget-msg-status hide'><span>发送失败</span><i></i></div>                                <div class='easemobWidget-msg-loading'>"+EasemobWidget.LOADING+"</div>\n                                <div class='easemobWidget-msg-container'>                                    <a class='easemobWidget-noline' href='"+file.url+"' target='_blank'><img src='"+file.url+"'/></a>                                </div>                            </div>                        </div>                    ")}var opt={id:msgid,file:file,apiUrl:(https?"https:":"http:")+"//a1.easemob.com",to:config.to,type:"chat",ext:{messageType:"img"},onFileUploadError:function(error){Easemob.im.Utils.isCanUploadFileAsync()?setTimeout(function(){var id=error.id,w=$("#"+id),a=w.find("img:last").parent();a.html("").append($('<i class="easemobWidget-unimage">I</i>')),w.find(".easemobWidget-msg-loading").addClass("hide"),me.scrollBottom()},0):swfupload&&swfupload.settings.upload_error_handler()},onFileUploadComplete:function(data){me.chatWrapper.find("img:last").on("load",im.scrollBottom)},success:function(id){$("#"+id).find(".easemobWidget-msg-loading").addClass("hide")},fail:function(id){var msg=$("#"+id);msg.find(".easemobWidget-msg-loading").addClass("hide"),msg.find(".easemobWidget-msg-status").removeClass("hide")},flashUpload:Easemob.im.Utils.isCanUploadFileAsync()?null:flashUpload};me.handleGroup(opt),me.send(opt),me.addDate(),me.chatWrapper.append(temp),me.chatWrapper.find("img:last").on("load",me.scrollBottom)},encode:function(str,history){if(!str||0==str.length)return"";var s="";return s=str.replace(/&amp;/g,"&"),s=s.replace(/<(?=[^o][^)])/g,"&lt;"),s=s.replace(/>/g,"&gt;"),s=s.replace(/\"/g,"&quot;"),s=s.replace(/\n/g,"<br>")},decode:function(str){if(!str||0==str.length)return"";var s="";return s=str.replace(/&amp;/g,"&")},sendTextMsg:function(msg,wrapper,isHistory){var me=this;if(wrapper=wrapper||me.chatWrapper,isHistory)return void wrapper.prepend("                        <div class='easemobWidget-right'>                            <div class='easemobWidget-msg-wrapper'>                                <i class='easemobWidget-right-corner'>J</i>                                <div class='easemobWidget-msg-status hide'><span>发送失败</span><i></i></div>                                <div class='easemobWidget-msg-loading hide'>"+EasemobWidget.LOADING+"</div>\n                                <div class='easemobWidget-msg-container'>                                    <p>"+Easemob.im.Utils.parseLink(me.face(me.encode(msg.msg,!0)))+"</p>                                </div>                            </div>                        </div>                    ");if(msg||me.textarea.val()){var txt=msg||me.textarea.val(),msgid=me.conn.getUniqueId();me.addDate(),wrapper.append("                    <div id='"+msgid+"' class='easemobWidget-right'>                        <div class='easemobWidget-msg-wrapper'>                            <i class='easemobWidget-right-corner'>J</i>                            <div class='easemobWidget-msg-status hide'><span>发送失败</span><i></i></div>                            <div class='easemobWidget-msg-loading'>"+EasemobWidget.LOADING+"</div>\n                            <div class='easemobWidget-msg-container'>                                <p>"+Easemob.im.Utils.parseLink(me.face(me.encode(txt)))+"</p>                            </div>                        </div>                    </div>                "),me.textarea.val(""),me.scrollBottom(EasemobWidget.utils.isMobile?700:undefined);var opt={id:msgid,to:config.to,msg:txt,type:"chat",success:function(id){$("#"+id).find(".easemobWidget-msg-loading").addClass("hide")},fail:function(id){var msg=$("#"+id);msg.find(".easemobWidget-msg-loading").addClass("hide"),msg.find(".easemobWidget-msg-status").removeClass("hide")}};me.handleGroup(opt),me.send(opt)}},send:function(option){var me=this,resend="string"==typeof option;resend?option:option.id;resend||sendQueue[curGroup]||(sendQueue[curGroup]=[]),me.conn.send(option)},sendSatisfaction:function(level,content){var me=this,opt={to:config.to,msg:"",type:"chat",ext:{weichat:{ctrlType:"enquiry",ctrlArgs:{inviteId:me.satisDialog.get(0).inviteId||"",serviceSessionId:me.satisDialog.get(0).serviceSessionId||"",detail:content,summary:level}}}};this.handleGroup(opt),this.send(opt)},addPrompt:function(detail){!this.isOpened&&this.msgCount>0?(this.msgCount>9?this.messageCount.addClass("mutiCount").html("..."):this.messageCount.removeClass("mutiCount").html(this.msgCount),message.sendToParent("msgPrompt"),this.notify(detail||"")):(this.msgCount=0,this.messageCount.html("").addClass("hide"),message.sendToParent("recoveryTitle"))},notify:function(detail){message.sendToParent("notify"+(detail||""))},receiveMsg:function(msg,type,isHistory,wrapper){var me=this,value="",msgDetail="";switch(wrapper=wrapper||me.chatWrapper,msg.ext&&msg.ext.weichat&&msg.ext.weichat.ctrlType&&"inviteEnquiry"==msg.ext.weichat.ctrlType&&(type="satisfactionEvaluation"),msg.ext&&msg.ext.msgtype&&msg.ext.msgtype.choice&&(type="robertList"),type){case"txt":msgDetail=msg.msg||msg.data,msgDetail&&(msgDetail=msgDetail.replace(/\n/gm,""),msgDetail=msgDetail.length>15?msgDetail.slice(0,15)+"...":msgDetail,value=me.face(Easemob.im.Utils.parseLink(me.encode(isHistory?msg.msg:msg.data,isHistory))),value="<p>"+value+"</p>");break;case"face":msgDetail="",$.each(msg.data,function(k,v){v.data=v.data.replace(/>/g,"&gt;"),msgDetail+=v.data,value+=0>v.data.indexOf("data:image")?v.data:'<img class="chat-face-all" src="'+v.data+'">'}),msgDetail=value.replace(/\n/gm,""),msgDetail=msgDetail.length>15?msgDetail.slice(0,15)+"...":msgDetail,value="<p>"+value+"</p>",value=Easemob.im.Utils.parseLink(value);break;case"img":value='<a href="'+msg.url+'" target="_blank"><img src="'+(msg.thumb||msg.url)+'"></a>',msgDetail="[图片]";break;case"satisfactionEvaluation":value="<p>请对我的服务做出评价</p>",msgDetail="请对我的服务做出评价";break;case"robertList":value="<p>"+msg.ext.msgtype.choice.title+"</p>"}var temp="                    <div class='easemobWidget-left'>                        <div class='easemobWidget-msg-wrapper'>                            <i class='easemobWidget-left-corner'>K</i>                            <div class='easemobWidget-msg-container'>"+value+"</div>                            <div class='easemobWidget-msg-status hide'><i></i><span>发送失败</span></div>                        </div>"+function(){var str="";switch(type){case"satisfactionEvaluation":str='<div class="easemobWidget-list-btn js_satisfybtn"><button data-inviteid="'+msg.ext.weichat.ctrlArgs.inviteId+'" data-servicesessionid="'+msg.ext.weichat.ctrlArgs.serviceSessionId+'">立即评价</button></div>';break;case"robertList":if(msg.ext.msgtype.choice.list.length>0){var list=msg.ext.msgtype.choice.list;str='<div class="easemobWidget-list-btn js_robertbtn">';for(var i=0,l=list.length;l>i;i++)str+="<button>"+list[i]+"</button>";str+="</div>"}}return str}()+"</div>";isHistory?wrapper.prepend(temp):(me.addDate(),wrapper.append(temp),me.resetSpan(),me.scrollBottom()),isHistory||(me.isOpened?EasemobWidget.utils.isMin()&&me.notify(msgDetail):(me.messageCount.html("").removeClass("hide"),me.msgCount+=1,me.addPrompt(msgDetail)))}}.setAttribute(),flashUpload=function(url,options){swfupload.setUploadURL(url),swfupload.startUpload(),swfupload.uploadOptions=options},handleGroupUser=function(name){userHash[name].user?$.when(userHash[name].password?null:EasemobWidget.api.getPwd({user:userHash[name].user}),userHash[name].group?null:EasemobWidget.api.getGroup({user:userHash[name].user,orgName:config.orgName,appName:config.appName,to:config.to})).done(function(info,group){config.user=userHash[name].user,group&&(userHash[name].group=group,im.chatWrapper.attr("data-group",group)),info&&(config.password=info,userHash[name].password=info),
im.open(),im.chatWrapper.data("hised")||im.getHistory(0,im.chatWrapper,function(wrapper,info){wrapper.attr("data-hised",1),config.history=info,im.handleHistory(wrapper),im.toggleChatWindow(isShowDirect?"show":"")})}):$.when(EasemobWidget.api.getUser(config)).done(function(info){config.user=info.userId,userHash[name].user=info.userId,config.password=info.userPassword,userHash[name].password=info.userPassword,config.root?Emc.setcookie(escape(name),config.user):message.sendToParent("setgroupuser@"+config.user+"@emgroupuser@"+name),im.open(),im.toggleChatWindow(isShowDirect?"show":"")})},message=(new EmMessage).listenToParent(function(msg){var value;switch(0==msg.indexOf("emgroup@")?(value=msg.slice(8),msg="emgroup"):msg.indexOf("@")>0&&(value=msg.split("@")[1],msg=msg.split("@")[0]),msg){case"dragend":im.scrollBottom();break;case"imclick":isGroupChat=!1,im.chatWrapper=$("#normal"),im.chatWrapper.removeClass("hide").siblings().addClass("hide"),config.user!=userHash.normal.user&&(config.user=userHash.normal.user,config.password=userHash.normal.password,im.open(),im.setTitle()),im.group=!1,im.toggleChatWindow(curGroup?"show":""),curGroup="";break;case"emgroup":isGroupChat=!0,isGroupOpened=!1;var idx=value.indexOf("@emgroupuser@"),user=null;idx>0&&(user=value.slice(0,idx)),value=value.slice(idx+13),userHash[value]=userHash[value]||{},userHash[value].user=user,curGroup!=value?(curGroup=value,isShowDirect=!0):isShowDirect=!1,im.handleGroup(value),handleGroupUser(curGroup),isShowDirect?im.toggleChatWindow("show"):im.toggleChatWindow()}}),uploadShim=function(fileInputId){if(Easemob.im.Utils.isCanUploadFile()){var uploadBtn=(document.title,$("#"+fileInputId));if(!("undefined"==typeof SWFUpload||uploadBtn.length<1))return new SWFUpload({file_post_name:"file",flash_url:"js/swfupload/swfupload.swf",button_placeholder_id:fileInputId,button_width:uploadBtn.width()||120,button_height:uploadBtn.height()||30,button_cursor:SWFUpload.CURSOR.HAND,button_window_mode:SWFUpload.WINDOW_MODE.TRANSPARENT,file_size_limit:10485760,file_upload_limit:0,file_queued_handler:function(file){this.getStats().files_queued>1&&this.cancelUpload(),EasemobWidget.PICTYPE[file.type.slice(1).toLowerCase()]?10485760<file.size?(im.errorPrompt("请上传大小不超过10M的文件"),this.cancelUpload()):(this.fileMsgId=im.conn.getUniqueId(),im.sendImgMsg(null,null,{name:file.name,data:file},this.fileMsgId)):(im.errorPrompt("不支持此类型"+file.type),this.cancelUpload())},file_dialog_start_handler:function(){},upload_error_handler:function(file,code,msg){if(code!=SWFUpload.UPLOAD_ERROR.FILE_CANCELLED&&code!=SWFUpload.UPLOAD_ERROR.UPLOAD_LIMIT_EXCEEDED&&code!=SWFUpload.UPLOAD_ERROR.FILE_VALIDATION_FAILED){var temp=$("                            <div class='easemobWidget-right'>                                <div class='easemobWidget-msg-wrapper'>                                    <i class='easemobWidget-right-corner'>J</i>                                    <div class='easemobWidget-msg-status hide'><span>发送失败</span><i></i></div>                                    <div class='easemobWidget-msg-container'>                                        <a class='easemobWidget-noline' href='javascript:;'><i class='easemobWidget-unimage'>I</i></a>                                    </div>                                </div>                            </div>                        ");im.chatWrapper.append(temp),im.chatWrapper.find("img:last").on("load",im.scrollBottom)}},upload_complete_handler:function(){},upload_success_handler:function(file,response){if(file&&response)try{var res=Easemob.im.Utils.parseUploadResponse(response);res=$.parseJSON(res),file&&!file.url&&res.entities&&res.entities.length>0&&(file.url=res.uri+"/"+res.entities[0].uuid);var temp=$("                            <div id='"+this.fileMsgId+"' class='easemobWidget-right'>                                <div class='easemobWidget-msg-wrapper'>                                    <i class='easemobWidget-right-corner'>J</i>                                    <div class='easemobWidget-msg-status hide'><span>发送失败</span><i></i></div>                                    <div class='easemobWidget-msg-loading'>"+EasemobWidget.LOADING+"</div>\n                                    <div class='easemobWidget-msg-container'>                                        <a class='easemobWidget-noline' href='"+file.url+"' target='_blank'><img src='"+file.url+"'/></a>                                    </div>                                </div>                            </div>                        ");im.chatWrapper.append(temp),im.scrollBottom(),this.uploadOptions.onFileUploadComplete(res)}catch(e){im.errorPrompt("上传图片发生错误")}}})}};!Easemob.im.Utils.isCanUploadFileAsync()&&Easemob.im.Utils.isCanUploadFile()&&(swfupload=uploadShim("easemobWidgetFileInput"),$('object[id^="SWFUpload"]').attr("title","图片"))};window.top==window?main(EasemobWidget.utils.getConfig()):(new EmMessage).listenToParent(function(msg){0==msg.indexOf("initdata:")&&main(EasemobWidget.utils.getConfig(msg.slice(9)))})}(window,void 0);