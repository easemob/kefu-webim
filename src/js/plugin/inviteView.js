...
module.exports = function(info){
	...
	window.localStorage && localStorage.setItem("visitTime", info.rule.visitTime);
	window.localStorage && localStorage.setItem("maxInvitation", info.rule.maxInvitation);
	...
	function onCancelClick(){
		var maxInvitation = localStorage.getItem("maxInvitation");
		hide();
		if(maxInvitation > 1){
			window.localStorage && localStorage.setItem("maxInvitation", maxInvitation - 1);
	...
	function startDayTimer(){
		...
		endDaySecond = (moment().endOf("day") - moment()) / 1000;
